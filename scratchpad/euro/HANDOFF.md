...
## v2.98.30(2026-09-01 01:15)— 三語齊平,歐陸批次收尾

最後 2 部簡中改走**繁中→簡中純翻譯**(`scripts/px_translate.py`),兩部一次合格,逐段把關通過。
正式站(DATA_VER `25ce04b623`)確認:`aggiungi un posto a tavola` 498 字、`a padlas` 408 字,
全部關鍵事實在位、無繁體殘留。

**最終狀態**
- 知識庫:en 51 / zh-hant 51 / **zh-hans 51**(三語齊平)
- 前端 served:各 48(差的 3 部 `Légy jó mindhalálig`、`A meseautó`、`Zrínyi 1566` 場次已結束,
  簡介仍在 library,有新場次會自動掛回)
- 已無「暫緩上架」項目;`gen/out_zhs_HELD.json` 保留作為當時的判斷紀錄

**方法論(這批最重要的三條)**
1. 判準只有一個:**這個製作自己的官方怎麼寫**。原著、電影、其他語言版本只能當對照。
