...
def build(g, e, idt, lang):
    ...
    if warn:
        ...
        # 🚨 2026-09-04:帳本的 🚨 限制句常寫「英文名照官方:黑桃女王=Queen of Spades」,
        #    那是【給英文版】的指示,但它排在 name_rule 之後,對中文版會蓋掉前面
        #    「中英並列時中文稿只用中文名」的規則 —— 《爱丽丝奇境之旅》繁中稿就因此
        #    寫出「不喜陽光的 Queen of Spades」。中文版要補一句把適用範圍講清楚。
        if lang != "en" and re.search(r"英文名|英文寫法|英文拼寫|羅馬字", warn):
            # 🚨 這句話【不可寫成「中文稿一律用中文名」】。本批有兩種相反的情形:
            #    (a)官方中英並列(Queen of Spades 黑桃女王、Edward 爱德华)→ 中文稿用中文;
            #    (b)官方【只給英文名、沒有中文譯名】(《#0528》的 Eggy/Brandon/Doris、
            #       《命运之上》的 JOHNNY/SHIRLEY、《风声》的 Turtle)→ 中文稿【必須保留英文】,
            #       帳本明令不可自創「艾吉/布蘭登/桃樂絲」。
            #    一刀切會逼模型去編中文譯名,反而製造新錯。所以只能重申分辨方式。
            body += ("⚠ 注意:上面關於【英文名／英文拼寫】的限制主要是給英文版用的。"
                     "中文稿的處理方式看資料怎麼給:同一個角色若【中英並列】就只寫中文名;"
                     "若資料【從頭到尾只給英文名、沒有中文譯名】,中文稿就保留那個英文原樣,"
                     "【絕對不可以自己造一個中文譯名】。")
    ...
    # 🚨 2026-09-04 全批掃描發現的系統性缺漏:官方開頭的「改編自 X / 以 X 為藍本」
    #    被生成稿【一律刪掉】(月亮与六便士的毛姆、牡丹亭的湯顯祖、花木兰的《木蘭詞》、
    #    觉醒年代的同名電視劇、日出渤海耀天山的史實依據,至少五組)。
    #    推定原因就是下面這句「不要提到這段資料本身」被擴大解讀成連改編來源也不能寫。
    #    但那類出處對讀者有實質資訊——它決定讀者把故事當史實還是虛構、當古典版還是迪士尼版。
    body += (" (不要提到這段資料本身,不要寫『根據官方』,不要用表格或小標題。"
             "⚠ 但資料若寫了【改編自某部作品／以某段史實或某個人的生平為藍本】,"
             "那是作品本身的出處,【要寫進簡介】,不算『提到資料本身』。)")
    return ident + " —— " + body
