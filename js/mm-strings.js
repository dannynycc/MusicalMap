...
(function () {
  var STR = {
    'zh-hant': {
      ...
      gate_secure: '用你的 Google 帳戶安全登入。我們只讀取名稱與 email（Google 授權會附帶頭像，本站不使用），不會代你張貼任何內容。',
      ...
      pp_s2_li1: '你的 Google 帳號基本資料——<strong>電子郵件與顯示名稱</strong>（透過 Google OAuth 取得，用於辨識你的帳號）。Google 的基本資料授權會將<strong>頭像</strong>與名稱一併提供（無法只取名稱），本網站不使用、不顯示頭像；',
    ...
    'en': {
      ...
      gate_secure: 'Secure sign-in with your Google account. We only read your name and email (Google bundles an avatar with the grant; this site doesn’t use it) — we never post anything on your behalf.',
      ...
      pp_s2_li1: 'your basic Google profile — <strong>email address and display name</strong> (obtained via Google OAuth, used to identify your account). Google’s basic-profile grant also bundles your <strong>avatar</strong> with the name (it cannot be requested separately); this site does not use or display it;',
