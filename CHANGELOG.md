...
## [v2.119.1] - 2026-09-05 00:55

### 核對三語覆蓋率產生器,抓到兩個錯(使用者要求「不要完全相信產生器的結果」)

重出全資料庫覆蓋率報表前,**先不看報表、自己從 `data/variants/` 與 `data/synopses/`
獨立重算一次**再對照。分母 629、英文標題 516、劇情 606×3 都吻合,但**中文標題差 1 組**。

**① 產生器把台式的「の」當成日文,少算 1 組。**
差的是台灣製作《**無聲の海腳間仔**》(漢字 6、假名 1,在臺灣演出)。
`script_of()` 只要看到**一個**假名就把整串判成日文,於是中文圈常見的
【の 當「的」用】的寫法被連坐誤殺。而真正的鑑別器**中文市場檢查**根本沒機會執行。
→ 新增 `zh_local_title()`:漢字**多於**假名 且 在中文市場有場次,才算中文在地題名。
全庫 18 組題名含假名,**17 組是日本的(不在中文市場)、只有這 1 組是中文市場**,
所以修正後差值**只有這一組改變**(繁中 389→390、簡中 388→389、三語全滿 313→314)。
保留「漢字 > 假名」的比例護欄,是為了擋日後日文題名巡演到中文市場的情況
(テニスの王子様 漢3假名4 → 仍不算,走官方譯名表那條)。

**② 報表裡有一整段是 2026-09-02 寫死的舊結論,現在已經變成錯的。**

| 寫死的句子 | 2026-09-05 的事實 |
|---|---|
| 「中國原創 **88 組**的劇情簡介是 **0%**」 | **75 組 / 73 有簡介 = 97%**,只缺 `q`、`冈格尼尔` |
| 「西葡另外 4 組被靜默漏掉」(mariana / es navidad / monstruos / leyendas…) | **這 4 組三語簡介都有了** |

**寫死的結論會從「當時的事實」變成「現在的錯誤」**,而且這份是要交到使用者手上的檔案。
→ 整段改成**由本次資料算出**的「缺口最深的分類」(列出各分類缺幾組、缺哪些 group)。
順帶把同一區塊裡「中文標題有+劇情無 → **多為中國原創**」那句也改成資料驅動——
那句當下剛好還成立(2 組確實都是中國原創),但只要缺口換分類就會變成假話。
另補一行口徑說明:分母是【目前在檔期】的作品組,會隨檔期進出變動,
不同日期的兩份報表數字不同是正常的。

### 交付

`Desktop/MusicalMap_三語覆蓋率_2026-09-05.html`(249 KB,自包含)。
**逐項核對過**:分母/三語標題/三語劇情/三語全滿 8 個數字全部與我獨立重算的一致;
四象限 388+218+2+21 = 629 對得起來;明細 629 列;零殘留 `__佔位符__`;
headless Chrome 實際截圖確認 render。
...
使用者指出《邦尼帮你》的 BONNIE BUNNY 與《嗜血博士》的 Doctor,兩個都屬實;
順手回頭重驗又發現《月亮与六便士》也錯。加上先前的 Back 與 Mu Lan,**這一類判斷 5 次全錯**。

| 組 | 官方題名鎖定裡印的 | 我原本的理由 |
|---|---|---|
| 邦尼帮你 | **BONNIE BUNNY**(中文劇名正下方;頁尾另有 THE MUSICAL BONNIE BUNNY) | 「頁面上的英文都是角色名(BUNNY /…)」——我看到 BUNNY 卻沒看出它是完整的 BONNIE BUNNY |
| 嗜血博士 | **Doctor**(頁首與中文劇名構成鎖定;大麥標題欄直接寫「嗜血博士 doctor」) | 「手寫小寫裝飾字,不是可獨立使用的題名」 |
| 月亮与六便士 | **Moon and Sixpence**(「音乐剧」正下方,🚨 沒有 The) | 「那只是原著書名 The Moon and Sixpence,不是本製作自訂的英文題名」 |

**共同的錯誤形狀:我把「這串英文看起來像裝飾/標語/原著書名」當成觀察,但那是【我的詮釋】。**
可操作的判準應該只有一條:**只要它出現在題名鎖定裡(疊在或緊貼中文劇名),就當官方英文劇名收下。**

唯一經得起重驗的反例是《觉醒年代》的 LA JEUNESSE——那是**法文**,而且是劇中道具
《新青年》雜誌的法文刊名(該雜誌史實上就用這個刊名),不在題名鎖定裡。已第二次重驗確認。

`show_titles_en` 現 **47 筆**,驗過 47/47 題名正確且進得了搜尋索引。

---

## [v2.118.0] - 2026-09-04 20:55

### 🚨 產地標錯:《三风一树》是韓國音樂劇,不是中國原創

使用者提供來源後查證 `saoju.net/yyj/musical/508` 逐字確認:
**【引进】【原版韩国】【版权授权:MBZ Company】【编剧 Rijun An】【作曲 Soohyun Huh】
【出品/制作:一台好戏文化传媒有限公司】** —— 是韓國小劇場音樂劇《아나키스트》(Anarchist)的中文版。

**⚠ 而我在前一輪就看到證據了,卻只記錄不查證。** 複查英文劇名時我在主視覺上讀到
韓文「아나키스트」與「Book by Riljun An / Music by Soohyun Huh」兩個韓國人名,
只寫進 `_origin_question` 說「尚未查證」就放著——**線索已經在手上,少的是動手去查那一步**。

已在 `data/works.json` 註冊 `tradition=韓國原創`(registry 優先於 country fallback)。
**刻意不放裸的 `Anarchist` 別名**,理由同本檔 Borderline 那條的教訓:
單一常見英文單字當別名會誤捕日後任何同名作品(如 David Mamet 的話劇 The Anarchist)。

**驗證做了差值比對而非最終狀態**:註冊前 `classify_tag('三风一树','damai','China')` = 中國原創,
註冊後 = 韓國原創;對照組「荒原」仍是中國原創(註冊沒有外溢);繁體別名與韓文原名都命中;
裸 `anarchist` 在 broadway/USA 與 opentix/Taiwan 情境下**都不會**被捕捉成韓國原創。
重跑 `build_shows` 後比對 2081 筆:**只有 1 處差異**(三风一树 的 tag),零新增零消失。

### 🚨 《去你的夏天》的「Back」就是官方英文劇名——我判錯了,使用者推翻得對

我原本把主視覺上那個橘色手寫花體 Back 判為裝飾元素,理由是「單一單字、與劇名語意對不上」。
**第二個前提根本錯了**:官方標語就寫著「動聽悅耳旋律,帶你**重回**華語樂壇金曲時代」
(本劇是用李偉菘/李偲菘——孫燕姿御用作曲——金曲的環境式駐演),Back 正對應「重回」。
回查詳情圖更發現,同一個橘色 Back **也印在入場須知頁 `_6.jpg` 的中文劇名上**,
與主視覺是**同一組題名鎖定、重複出現在多份官方物料**,不是單張裝飾。

⚠ 我的具體錯誤是把它跟《嗜血博士》那個貼在字**旁**的小寫 doc/tor 混為一談,
但 Back 是**疊在題名上、與中文劇名構成一組**,設計處理完全不同。
`show_titles_en` 現 44 筆,驗過 44/44 題名正確且進得了搜尋索引,繁中站題名未被英文取代。
...
使用者逐一指出:《喜欢你》右上角有 I like you、《玉良》有 Musical Pan Yu Lin、
《空中花园谋杀案》有 The Murder of Hanging Garden、《花木兰》有 Mu Lan ——
四張我都親自下載確認,**全部屬實**。

**根因不是看漏,是抓圖只收了一半。** 我的抓圖只收 `imgextra` 型的【詳情圖】,
從來沒收過大麥商品頁的【主視覺海報】——全批 82 組 608 張圖裡,
`/bao/uploaded/`(主視覺型)**0 張**。而主視覺正是英文劇名最常印的地方。
更難堪的是:**主視覺 URL 一直躺在我們自己的 `data/china_damai.json` 的 `image` 欄裡**,
400 筆全都有,我從頭到尾沒去看過。

重抓 35 組共 **105 張主視覺**逐張看完,**35 組裡我判錯 5 組**(錯誤率 14%):

| 組 | 官方主視覺上印的 | 我原本的判定 |
|---|---|---|
| 喜欢你 | **I like you** | 「頁面文字無英文劇名 → 確認沒有」(9 張主視覺**每一張**都有) |
| 玉良 | **Pan Yu Lin**(上方小字 Musical) | 「尚未逐張看主視覺 → 未確認」 |
| 空中花园谋杀案 | **THE MURDER OF HANGING GARDEN** | 「詳情圖全為劇照、無文字 → 確認沒有」 |
| 渡河 渡河 | **The Other Shore** | 「11 張詳情圖逐張看過,官方沒有給英文題名」 |
| 花木兰 | **Mu Lan** | 「🚨【查無】…⚠ 迪士尼 Mulan 絕不可拿來當本劇英譯」 |

**🚨 其中《玉良》最嚴重:我帳本自己就寫著「尚未逐張看主視覺 → 標為未確認」,
我卻把它歸進「官方【確實】沒給」那一堆——明知沒查完卻當成查過了。**
《花木兰》次之:我一邊斬釘截鐵寫 🚨【查無】、一邊警告別用迪士尼的 Mulan,
而官方海報上就印著 Mu Lan。

前四組已加進 `show_titles_en`(現 43 筆),驗過 43/43 題名正確且進得了搜尋索引。

### ⚠《花木兰》有英文名但【不收】——它屬於另一個製作

group「花木兰」底下是兩個**不同的**製作:南京江蘇大劇院 2026-08-07~08 的
**兒童打擊樂音樂劇**(海報有 Mu Lan,已結束、不在目前 catalog),
以及商洛丹鳳縣 2026-10-05 的音樂劇(海報**沒有**英文,這才是現行那一筆)。
掛上去會讓現行那齣戲顯示另一齣戲的英文名。
⚠ 這也暴露更上游的問題:「花木兰」是常見題材,把不同製作併成同一 group 本身需要重新檢視。

### ⚠ 兩則記錄但不行動的觀察

- **《三风一树》可能不是中國原創。** 查英文劇名時看到:主視覺的拉丁字只有 THE MUSICAL
  (類別標籤),但其下有**韓文「아나키스트」(Anarchist)**,海報底部印
  **Book by Riljun An / Music by Soohyun Huh**(兩個韓國人名)。目前 tag 是中國原創。
  🚨 **我還沒查證,只記錄不改 tag**(產地必逐部查,不可批次推定)。
- **《去你的夏天》主視覺有一個橘色手寫花體英文單字「Back」**疊在中文劇名上(放大 5 倍確認)。
  判為裝飾元素而非劇名(單一單字、與劇名語意對不上,性質同《嗜血博士》的 doc/tor)。
  ⚠ 這是我的判斷不是官方陳述,已列進複查頁供推翻。

### 先擋再抓:這次沒有重蹈 74MB 覆轍

下載那 105 張主視覺**之前**先把 `scratchpad/cn82/poster/` 與 `_recheck/` 寫進 `.gitignore`,
並用**兩道**檢查確認(`git check-ignore` 擋新檔 + `git ls-files | grep 圖副檔名` 必須 0)。

### ⚠ 尚未做:我說「有英文名」的那 41 組也該用主視覺重驗

它們的英文名全部來自**詳情圖**,主視覺可能印著不同或更完整的題名。同一個缺口,還沒補。

---

## [v2.116.4] - 2026-09-04 18:12

### 35 組「官方沒給英文劇名」的人工複查包(輸出到桌面,不進 repo)

使用者要親自複查我判定為「官方沒有英文劇名」的那 35 組。新增
`scratchpad/cn82/mk_check_page.py`,產出到桌面
`MusicalMap_35組無英文劇名_2026-09-04/`(35 組 / 257 張原圖 / 239 MB)。
🚨 圖是爬下來的商業素材,**只放桌面、不進 repo**。

**為什麼需要這一頁:**「讀圖」是整條鏈上最主觀、最沒有第二道把關的一步。
同一批裡《当你心动时》海報明明印著 Heart Shake、我判定為「有」卻沒接進管線,
是使用者一眼看出來的。所以「判定為**沒有**」的這 35 組更需要人親自看。
頁面每組攤開三樣:完整售票連結、**我當初的判定依據逐字照錄**、我手上全部的存圖。

### 🚨 做這一頁時抓到我自己的判定依據寫得不準

`0528` 我寫「各區塊上方的英文**只是**欄目標籤(SYNOPSIS、CAST、CREDITS)」——
不準確:`0528_3.jpg` 上還有一句英文**標語**「Let's do it right now!」。
結論(官方沒有英文**劇名**)不變,但「只是欄目標籤」漏掉了標語。
→ **教訓:判定依據要寫「我看到什麼」,不要寫「只有什麼」**,後者是全稱斷言,一個反例就假。
已把這段自我更正寫進帳本,複查頁上也看得到。

### ⚠ 有 4 組的判定證據較弱,頁面上明確標紅

`我那长乐塬`、`羊角尖`、`花木兰`、`貂蝉 你在想什么` 不是大麥來源(聚橙/保利),
當初是讀**文字頁**做的判定,**沒有逐張看過主視覺**——頁面上直接寫明這四組結論證據較弱。

### 驗證

縮圖全部**內聯 base64**(桌面 HTML 用外部路徑常常整頁空白);
每張縮圖連到同資料夾的原圖,**257 個相對連結實測 0 個連不到**;
headless Chrome 實際截圖確認 render 正常(不是只確認檔案寫出來)。

---

## [v2.116.3] - 2026-09-04 18:03

### 🚨 74 MB / 129 張爬下來的韓國商業海報,一直留在公開 repo 的【現行版本】裡

使用者問起「歷史裡的商業圖」時查出來的,而且**比原本以為的嚴重**:不是躺在舊 commit,
是 `HEAD` 現在就有,GitHub 上任何人都取得到。

**成因是 gitignore 的經典盲點:`.gitignore` 對【已經被追蹤的檔案】完全無效。**
`scratchpad/kr/img/`、`scratchpad/kr/poster/` 在 `.gitignore` 第 27–31 行寫得好好的,
`git check-ignore` 也回「已忽略」——但那些檔案是在 `v2.111.1` 就 commit 進去的,
gitignore 是**事後**才加,所以擋的只有「之後的新檔」。

**🚨 這也暴露我自己的檢查方法有盲點。** 今天我對 cn82 做的 `git check-ignore` 驗證,
只證明「新檔會被擋」,不證明「舊檔沒進去」。cn82 我另外數了索引=0 張所以確實乾淨,
但我**從來沒對 kr 做過同一個檢查**。→ 正確的驗法是**兩個都要**:
`git check-ignore <檔案路徑>`(擋新的)+ `git ls-files <目錄> | grep 圖副檔名`(查舊的)。

本次先做**可逆的止血**:`git rm --cached` 把 130 個檔從索引移除(磁碟上的檔案不動,
網站零引用、不受影響)。⚠ **這不等於清除**——blob 仍存在於舊 commit 中,
從 GitHub 的舊 commit 頁面依然抓得到。真正清除需要改寫歷史 + force push,
那是不可逆操作,等使用者決定。

`.gitignore` 第 26 行本來就寫著「圖本身不需要保留在版控裡」,意圖一直是對的,只是沒生效。

---

## [v2.116.2] - 2026-09-04 17:55

### 查證帳本補記(無網站變動)

`scratchpad/cn82/verify_log.json`:把《寻找李二狗》三語的紀錄從「待重做」更新為
**重做後的實際通讀結果**(逐項對照補正後的來源,並與官方卡司排期表交叉印證),
並記下重做後又修的一處——三語都把「勳章串聯【劇情】」這個敘事結構寫成
角色在劇中跟著勳章找線索。最終:**76 組 / 228 篇 → ok 120 / fix 108**。

`scratchpad/cn82/ledger.json`:`嗜血博士` 與 `月亮与六便士` 加上 `_title_en_not_shipped`,
寫明為什麼有英文字卻不進 `show_titles_en`,避免下一輪又被當成「漏收」加回去。

---

## [v2.116.1] - 2026-09-04 17:48

### 🚨 我收集了 41 個官方英文劇名,卻從來沒接進管線

使用者看著《当你心动时》的卡片問:海報上明明印著 Heart Shake,為什麼英文站顯示不出來?
查下去是**我的漏做**:這一批逐張讀圖時,`title_en` 我 76 組全都收進帳本了
(41 組有、35 組官方確實沒給),但**只留在帳本裡**——`data/` 全域搜「Heart Shake」是 0 筆。

英文站的題名走 `gen_variants.mjs` 的 `enTitle(s)` → `s.title_en`,而那個欄位來自 scraper。
**大麥抓不到它,因為這些英文名只印在海報美術裡**,純文字爬蟲讀不到——這正是我當初
要逐張讀圖的原因,結果收完沒接上。

新增 `data/i18n_maps.json` 的 `show_titles_en`(39 筆),與既有 `show_titles`/`show_titles_tw`
同一個模式:**手工維護的源頭檔,下次重抓 scraper 不會被洗掉**
(寫進 `data/china.json` 那類產物是沒有用的)。

**🚨 沒收的兩組,理由記在帳本 `_title_en_not_shipped`:**
- `嗜血博士` 官方海報確實有英文,但那是**手寫小寫裝飾字**(拆成 doc/tor 兩行貼在中文劇名旁),
  不是可獨立使用的題名——英文站顯示成一齣叫「doctor」的戲會誤導。放大海報確認後排除。
- `月亮与六便士` 的 The Moon and Sixpence 是**原著小說書名**,不是本製作自訂的英文題名。
- 另外 35 組官方確實沒給,一律不收。**絕不自創**:《花木兰》不可拿迪士尼 Mulan、
  《黑旗令》不可自造 Black Flag Order。

### 🚨 同一次修改裡抓到第二個問題:題名顯示得出來、卻搜尋不到

`buildSearch()` 原本寫成 `if (s.title_en) { ... enTitle(s) ... }`——**只在 `title_en` 非空時
才呼叫 `enTitle()`**。而這 39 組的 `title_en` 正好是空的,英文名進不了搜尋索引。
畫面看得到 ≠ 操作得到。已改成無條件呼叫,並驗過 39 組 × 3 語:題名正確、
英文名在搜尋索引裡、原本的中文名也還在(繁中站題名**未**被英文取代)。

---

## [v2.116.0] - 2026-09-04 17:27

### 中國原創批次完成:76 組 × 三語 = 228 篇簡介入庫上線

帳本 76 組全部建完(69 主批 + 7 補充),生成 228 篇,**每一篇都由本人逐字通讀**,
逐條判定與依據留在 `scratchpad/cn82/verify_log.json`:**ok 120 / fix 108**。
知識庫 605 → 681 筆/語,前端 served 604 筆/語,三語組別完全一致。

### 🚨 帳本抓錯區塊,《寻找李二狗》整組資料是錯的

通讀時發現**內部矛盾**:劇名叫《尋找李二狗》、官方卡司表也有「李二狗」這個角色,
稿子卻**整篇沒有他**。回頭重抓百度百科才發現,我存進帳本的是同一頁的
**「艺术特色」段而不是「剧情简介」段** —— 真正的前提
「當代青年常衛國因爺爺的回憶錄穿越至 1950 年鴨綠江畔,與志願軍戰士李二狗相遇」
根本沒餵進去。缺的不是細節而是**前提**;模型沒有憑空補(那是對的),
所以稿子只能停在抽象的「一名現代年輕人」。

依「修症狀必全庫掃同症狀所有變型」,把同樣取自百度百科的另外四組全部重抓比對:
**杜十娘、黑旗令的「剧情简介」與帳本逐字相符**;南墙计划、当你心动时 用的段名是
「内容简介」不是「剧情简介」,不是抽錯。→ 只有這一組有問題,補正後三語重新生成。

### 🚨 繁體字混進簡中稿(只有一篇,靠全掃才抓到)

`ne zha` 簡中寫出「鬧海抗爭」「蓮花重生」。成因不是模型亂寫,而是**帳本裡這一組的
官方資料是用繁體存的**,而 prompt 又要求「專有名詞一字不改照抄」,模型照做了。
所以這個錯**只會出現在資料來源是繁體的組**,其他 75 組一個都沒有 —— 抽樣抓不到。
新增 `scripts/zhs_trad_scan.py` 做全掃,並在檔內寫明 OpenCC 的誤報
`吒→咤`(哪吒繁簡同形,跟著轉反而錯)。

### 新增 `scratchpad/cn82/dup_scan.py`:用詞重複掃描

順序是**先親自讀出這一類問題**(牡丹亭同句兩次「傳奇」、日出渤海相鄰兩句
「渤海軍區教導旅」、ne zha 三個情節座標列兩次),**才把它寫成檢查**,不是反過來。
掃出 5 處整句逐字重複(荒原「復仇並非終點」相鄰兩段、蝶变四項列舉重複、
我那长乐塬「工業火種」同段兩次、旧讼、杜十娘)已修。
收尾段呼應主題詞是 prompt 要求的結構,不是瑕疵,一律不動。

### 🚨「修 A 引入 B」累計第六次 —— 這次全部在交卷前抓到

簡中第二輪重讀找到 8 個問題,**其中 7 個是我自己插入的句子造成的**:
「原著為李天濟」(原著是作品不是人,而且繁中犯過同一個錯已修、我補簡中時又寫回破的版本)、
玉良「藏着…的刺破,以及…为何震动民国」名詞片語與疑問子句無法並列、
0528 刪掉憑空補的地點後整句沒了謂語、牡丹亭同句兩次「傳奇」。
連我修 `ne zha` 時自己又引入「在當代舞台上」重複,也在下一輪讀到。

### 查證分級如實登錄,不把官方單一來源說成多源交叉

`data/synopses_verification.json` 新增 76 組。新增兩個 method 值並在 `_note` 定義:
**B-official 70 組**(事實來源是官方物料,單一來源,`external_multisource` 一律 false)、
**B-external 6 組**;真正 `external_multisource=true` 的**只有 4 組**。
兩者共通點是「Perplexity 生成為主體,再由本人逐語言逐篇通讀比對」——不是 agent 代讀。

### 其他修正

- 英文《当你心动时》拼字錯誤 `unsettined`→`unsettled`(守門抓不到,只有逐字讀才看得到);
  又把「化為人形」的小白寫成 `non-human embodiment`,與來源相反。
- 英文《南墙计划》兩處事實變形:來源的「反抗」寫成 `seeks to answer his father's
  expectations`(方向相反);「父親安排的封箱演出」寫成父親自己的告別演出。
- 《杜十娘》簡中漏了改編出處(馮夢龍),繁中也漏 —— 三語不一致,已補齊。
- 修好兩支自己的工具:`verify_log.summary()` 與 `st.py` 沒排除底線開頭的註記鍵會當場炸掉;
  `apply_fix.py` 加上 `suf` 欄位才認得補充批的檔名(否則會靜默套到主批檔上)。

### 驗證

- 9 個 keymap **100% 命中**;`kb_merge` 九次執行**全部覆蓋 0**,228 筆對應行落在
  76 個真 group 上、每個鍵剛好 3 次(三語各一)、**零垃圾鍵**。
  ⚠ `kb_merge.py:62` 每次執行都會清空 log,所以逐行讀 log 前必須逐次收集。
- 1149 張商業圖在磁碟上、**git 索引裡 0 張**(依鐵則用檔案路徑驗證,非目錄路徑)。

---

## [v2.115.9] - 2026-09-04 16:12

### 簡中通讀進行中(生成 55/69,已讀 51 篇);收尾步驟寫成 `scratchpad/cn82/FINISH.md`

**🚨 同一個錯第二次,責任在我。** `狐说臣与仙` 繁中稿當初產出「這屬於典故背景上的推斷,
並非劇中明說」,我**只修了繁中文字、沒修帳本裡那句指示**(「可點出典故,但要標明這是官方
沒有明說的推斷」)——簡中 prompt 照樣帶著它,於是又寫出「可視為未被明言的典故背景」。
這正是「手改要改在管線源頭檔,不是下游輸出」。
⚠ 而且我的**第一版更正說明也放在 `characters` 欄**,裡面還引用了那兩句錯誤示範——
`mk_regen` 會把含 🚨 的句子抽進 prompt,等於把要禁的東西又餵回去。已改成 `characters`
只留一句絕對禁令、原委另存 `_allusion_incident`,並驗過新 prompt 不再有任何含糊指示。

### 🚨 一個被推翻的懷疑,以及由它得到的新檢查標準
`时光代理人` 簡中寫「改编自哔哩哔哩同名国产动画」,那句**不在 `official_plot` 裡**,
我判定是幻覺、準備刪除。查證後推翻:**prompt 的身份段本身就寫著它**,帳本 `note` 也記著
「改編自 bilibili 原創動畫」是我從官方物料讀來的。
→ 新標準:**「不在 official_plot 裡」不等於「不在我給模型的資料裡」**,prompt 還包含身份段。
用這個標準**回查我判成「憑空補」的 7 條**(0528 的街角、四大美女的千年前、六个说谎的大学生的
六人重聚、我那长乐塬的 unnamed、命运之上的 SHIRLEY 對應、去你的夏天的性別、边城的 the four)
——**全部成立**。`cmp3.py` 已改成一併印出身份段,判斷前必看。

### 三個未結的產地問題全部結案(都維持中國原創,`works.json` 不需覆寫)
- `流星之绊`:saoju 明列「改編自東野圭吾同名小說 **官方授權 株式會社講談社**」,但出品製作與
  主創全為中方,媒體稿寫「以**中文原創**環境式音樂劇的形式」→ 中方創作 + 日方 IP 授權,
  與《月亮与六便士》《海上钢琴师》《时光代理人》同型態;跟《桑塔露琪亚》那種**整齣韓國
  音樂劇引進翻譯**方向不同。
- `玉良`:確認為**香港演戲家族**製作(官網專案頁 + 2025-04 荃灣大會堂階段演出 + 2026-09 香港
  中華文化節)。
- `有一天我和祝英台去美术馆`:**非常林奕華**(香港劇團)第 76 齣作品。
- 後兩者依使用者先前對《金童子》《边城》的裁定,香港作品留在中國原創並於帳本註記。

### 入庫路徑【五道】預驗全過(在真的跑 kb_merge 之前)
keymap 三語各 69/69 命中 / 69 個 group 全在目錄 621 個 group 裡 / 庫裡 605 筆與本批 76 組
**零重疊**(純新增) / `SUB` 子鍵映射與庫實際結構完全一致 / `build_served_synopses` 的小寫過濾
不會濾掉任何一組(庫、目錄、本批都沒有大寫字母)。
🚨 為什麼要事前驗:這五道任何一道錯了,結果都是「跑完看起來成功、實際什麼都沒上線」。

### 簡中通讀新增的 fix(累計 21 條)
`日出渤海耀天山` 掉了熱合曼的**維吾爾族身分**與阿里木是**熱合曼後人**(本劇主題就是民族團結);
`桑塔露琪亚` 掉了 Chi Chi 是**前任老大的另一個兒子**;`昨日显影` 把「以愛為名的控制,
**比單純的恨意更為殘忍**」砍掉後半;`破墙` 官方三個連續追問只剩一句;`玉良` 掉了**安徽博物院
四千餘件遺作**;`荒原` 收尾寫「重逢」但官方只說「重返故地」;`花木兰` 的改編出處變成懸垂修飾。

⚠ 也記到幾個**簡中做對、其他語言當初做錯**的:`有一天…美術館`(兩個坑都避開)、
`嗜血博士` 與 `爱丽丝奇境之旅`(用中文名,證明我改的 prompt 規則生效)、`怨种闺蜜`
(正確寫「守護對方最深的痛」)、`渡河 渡河`(沒有寫出評論官方文本的後設語言)。

**狀態**:簡中未完成、補充 7 組未生成;**尚未入庫、尚未部署。**

---

## [v2.115.8] - 2026-09-04 15:46

### 🚨 抓到一個會讓整批入庫【靜默失效】的 bug:英文 keymap 命中 0/69

`kb_merge` 對每筆結果做 `keymap.get(title) or group_key(title)`,而 title 是【整段重生成 prompt】。
keymap 對不到時**不會報錯**,只會改用 `group_key(整段 prompt)` 算出垃圾鍵——
207 篇簡介會全部掛在不存在的 group 上,前端永遠不顯示。

**原因是我自己的動作**:為了做 mk_regen 的反向測試,我重跑了 `mk_regen.py en`,
把 `regen_en_list.json` 覆蓋成【新版 prompt】;而 `regen_en.json` 的 `show` 欄存的是
【生成當時的舊 prompt】。keymap 從清單檔建,兩者一漂移就全毀。

**兩道修正**:
1. `mk_keymap.py` 改成從【kb_merge 實際會讀的那個檔】(`regen_<suf>.json` 的 show 欄)建 keymap,
   不再從清單檔建。修完三語各 69/69 命中;`keymap_en.json` 與 v2.115.6 committed 版本**完全相同**,
   反證修法正確。
2. `mk_regen.py` 加【輸出後綴】參數,子集清單重跑不會再覆蓋主清單檔。

**另外驗了入庫路徑的另一半**:這 69 個 group key 在 `data/shows.json` 的 621 個 group 裡
**全部存在**(對不到 0 個)。

### 原本沒有簡介的 9 組:7 組找到來源,2 組如實維持無簡介
這 9 組官方【沒有劇情簡介區塊】。逐組查外部後:
- **查到製作方劇情(4 組)**:`南墙计划`(百度百科+搜狐+豆瓣,並解開官方卡司的歧義——
  【小六】與百度寫的【张承艺】是同一人)、`寻找李二狗`(百度百科+人民日報+騰訊,三源一致)、
  `黑旗令`(百度百科+搜狐+中新網,三源一致)、`当你心动时`(百度百科+豆瓣)
- **外部單一來源但有官方卡司互證(1 組)**:`杜十娘`(百度百科全文劇情,四個人名與官方角色海報完全對得上)
- **官方物料本身就有情節(1 組)**:`画中人` —— 官方角色頁載明「前尚書之子家道中落流落醉仙樓賣畫為生、
  捲入魂約」等,**不需外部來源**
- **官方轉載的媒體描述(1 組)**:`ne zha`(官方引用新華社的「靈珠轉世/鬧海抗爭/蓮花重生」三個情節)
- 🚨 **查不到、如實維持無簡介(2 組)**:
  - `冈格尼尔`:搜到的是**同名 2023 年電影**(洛杉磯男孩里奧與女孩夏莉的疫情故事)與
    **北歐神話奧丁的槍** —— 都與本劇無關,正是「資料真空時拿題材相近的東西頂替」的陷阱
  - `q`(阿Q与吴妈):只有「改編自魯迅《阿Q正傳》」的宣傳語,以及**小說本身**的情節。
    官方從未說取用哪些段落、如何取捨,**小說情節不等於本劇情節**

⚠ 每組的來源、層級與保留意見都逐條寫進帳本(例如 `当你心动时` 百度提到的【魔女莉莉丝】
不在官方三角色卡上,已標明不可寫入;`杜十娘` 的外部劇情寫到了結局而官方沒有)。

### 🚨 第五次:我的編輯說明差點又漏進 prompt
`画中人` 的事實欄位第一版被我寫成「官方沒有劇情簡介區塊,但角色頁本身載有足夠的情節資訊
(逐字照抄,非我改寫):…」,而 `mk_regen` 會把整個 text 原封餵進 prompt。
生成前發現並清掉(連【】強調標記一起),並掃過其餘 6 組確認乾淨。

### 新增 `scripts/zhs_quote_norm.py`(簡中引號正規化,5 項正反向測試)
大陸官方物料兩種引號都用,模型照抄,結果同批內部不一致:`入傩`/`去你的夏天` 用 “”、
`亡灵之旅`/`六个说谎的大学生` 用「」,而 `博物馆奇遇记` **同一篇裡兩種都有**。
慣例從庫裡數出來:zh-hans 的 “ 848 次 : 「 294 次。
🚨 加了防呆:對繁中檔會拒跑(繁中慣例相反),實測 exit 2。

### 簡中通讀進行中(生成 26/69,已讀的全部讀完)
目前 8 條修正,兩條較重要:
- `命运之上` 把 **SHIRLEY=女孩、JOHNNY=巨星寫死對應** —— 帳本明令官方沒說哪個對應哪個
- `去你的夏天` 用「他」寫死游亦性別 —— 官方從未交代(英文版當初犯同一個錯,繁中版沒犯)
- `四大美女` 把官方的「**上一世**由美貌引發的悲劇」+「遊蕩了**千年**」併成「**千年前**的悲劇」,
  但四大美女分屬不同朝代(西施西元前五世紀、楊玉環八世紀),千年前對誰都不準
- `女巫` 出現與繁中**完全相同**的弱化(官方「即將」→「可能」),兩語各自獨立犯同一個錯,
  是模型的避險傾向而非隨機失誤

⚠ 也記到兩個【簡中做對、繁中當初做錯】的:`嗜血博士` 用中文名喬納森/愛德華
(證明我改的 prompt 規則生效)、`怨种闺蜜` 正確寫「守護對方最深的痛」。

**狀態**:簡中未完成、補充 7 組未生成;**尚未入庫、尚未部署。**

---

## [v2.115.7] - 2026-09-04 15:18

### 中國原創批次:繁中+英文各 69 篇全部生成並【逐句人工通讀完畢】,84 條修正已套用

**狀態(如實)**:繁中 69/69、英文 69/69 生成完成;**通讀 138 篇 / 69 組**(ok 76 / fix 62);
84 條修正涵蓋 44 組(繁中 42 / 英文 42)已寫回。
**簡中(zh-hans)剛開始生成**,尚未通讀。**尚未入庫、尚未部署。**

**使用者兩次指出「你可以更認真自己看,不要把主控權全交出去 / 不要太依賴 parser」。**
第一次之後改成逐句人工對讀,第二次之後把先前判「屬省略不屬錯誤,保留」的**全部翻回來重判**。
結果 fix 從 24 升到 62 —— 之前確實放水了。以下是**守門完全沒響、只有人工讀出來**的例子:

- `怨种闺蜜` 繁中把官方「**保護**彼此最深的痛」寫成「把痛**藏**起來不讓對方知道」,方向相反
- `风声` 繁中把官方「引領**觀眾**穿越時空」寫成「**眾人**穿越時空」——諜戰劇被寫成穿越劇
- `有一天我和祝英台去美术馆` 繁中刪掉官方開頭「**十二年前**…」只留「十二年後」,時間起算點消失
- `六个说谎的大学生` 英文末句憑空補「六個學生被拉回面對過往」(官方沒說重聚,且幕後黑手已死)
- `貂蝉` 英文把「**司徒**」譯成 Grand Commandant(那是**太尉**);查 en.wikipedia 王允條目
  infobox 確認司徒=Minister over the Masses
- `边城` 英文寫 "Fate brings **the four** together",官方寫的是「緣分讓**大家**相遇」**沒給人數**
- `女巫` 英文寫 Abby 但**官方卡司明寫 Abbey**;`savior` 寫 Pille,查證史實為 Joseph Piller
- `羊角尖` 英文把劇名拼成 Yangjia**jian**,漏一個 o(pypinyin 核對 yang-**jiao**-jian)
- `玉良` **兩個語言都**掉了官方第一句「中國首位女西洋畫家」——她之所以重要的理由
- `风声` 英文把「老鬼」音譯成 Lao Gui,但官方自己的慣例是意譯(角色表明寫**老鳖 = Turtle**)

**錯的源頭在我自己的三類**:
1. **帳本措辭不精準**:`拜风` 我寫「阿江/阿风 合起來就是劇名『拜风』」——實際是「江風」,
   英文照抄成 `Their names together form Bai Feng`。已掃過帳本全部 12 句斷言,只有這句錯。
2. **帳本註記漏進正文(四次)**:`心安东坡` 的 `【】`、`狐说臣与仙` 整句「這屬於典故背景上的推斷,
   並非劇中明說」、`渡河 渡河` 英文的 "though only three figures are identified"、`青春之歌` 四組 `【】`。
3. **prompt 造成的系統性缺漏**:官方開頭的「改編自 X／以 X 為藍本」被**一律刪掉**,
   確認七組(毛姆、湯顯祖、《木蘭詞》、同名電視劇、渤海軍區史實、Martin Booth 回憶錄、楊沫)。
   源頭是我寫的「不要提到這段資料本身」被擴大解讀。已修 mk_regen 並全部補回。

**修正套用的驗證(三道)**:
1. `apply_fix` 的斷言擋下了**我自己寫錯的一條**(`风声` 的 old 是 "Qiu Zhuang in Hangzhou" 但 n 設成
   "Qiu Zhuang" 的 2 次)。因為斷言在寫檔前,**一個字都沒寫進去**,已驗證原檔完全未動。
2. 15 項雙向抽驗全過(舊字串不在、新字串在)。
3. **cn_gate 前後差異比對**:消失的告警全是我修掉的(【】外洩×2、中英混寫×2、編輯指示外洩、
   卡司名闖入);新增的告警沒有一個是真問題(`灯塔` 從「拼音對得上」變「對不上」正是因為
   `Huo Pu`→`Hope`,守門不可能知道霍普=Hope)。

**修 A 引入 B 的自我檢查(第二輪修正 4 條)**:重讀後發現我的修正自己造成三個問題——
`日出渤海耀天山` 兩語都變成「渤海軍區教導旅」相鄰兩句各出現一次、`牡丹亭` 開頭變成在介紹
**原著戲曲**而非本劇、`金童子` 同一人先寫「馬丁·布斯」後寫「Martin」。已全部再修並重讀。

### `scripts/zht_glyph_norm.py` 擴充(31 項正反向測試全過)
新增 `身份→身分`(全站 298:2)、`閤→合`(全站 339:0,PROTECT 閤家/閤第/閤府)、
`迴歸→回歸`(全站 4:0,但**迴避 7:0、蕩氣迴腸都對所以不可整字替換**)、
`嶽州→岳州`(zh-tw 維基 岳州 23:嶽州 0,保護五嶽/山嶽/嶽麓)、`船伕→船夫`(全站 夫 398:伕 2)。
⚠ **查過但刻意不收**:`嚐/嘗` 是語意分工(嚐=品嚐、嘗=嘗試)不是異體,收了會改壞。

### `scratchpad/cn82/mk_regen.py` 三項 prompt 修正(供簡中與後續批次使用)
1. 中英並列名的中文寫法規則;2. 改編來源要寫進簡介;
3. 帳本的「英文名照官方」限制句只適用英文版 —— ⚠ 這條第一版我寫成「中文稿一律使用中文名」,
   **會逼模型替《#0528》的 Eggy/Brandon/Doris 編造中文譯名**(帳本明令不可),已改成分辨兩種情形。

### `scratchpad/cn82/apply_fix.py` / `st.py`
`apply_fix` 加了 px_gen 執行中拒寫的守門(實測 exit 2)與「原字串出現剛好 N 次」斷言;
`st.py` 是一行式進度(verify_log.summary 已太長)。

---

## [v2.115.6] - 2026-09-04 14:14

### 中國原創批次:逐句人工通讀 80 篇,守門補四項反向檢查

**進度(如實)**:生成 繁中 44/69、英文 38/69(仍在跑);人工通讀 **80 篇 / 44 組**
(ok 56 / fix 24);修正累積在 `scratchpad/cn82/fixes.json` 共 33 條,
**尚未寫回、尚未入庫、尚未部署**。簡中(zh-hans)整批尚未開始。

**這一輪最重要的一件事**:使用者指出我太依賴 parser。改成逐句人工對讀之後,
**守門完全沒響、只有人工讀出來的問題超過 20 個**,例如:
- `六个说谎的大学生` 英文末句憑空補「六個學生被拉回面對過往」——官方沒說重聚,
  且幕後黑手已死,六人一起被拉回連人數都不成立
- `去你的夏天` 英文三處用 his/he 寫死性別,官方只寫「一名成熟的社畜」
- `女巫` 英文寫 Abby,但**官方卡司明寫 Abbey**;`savior` 寫 Pille,查證史實為 Joseph Piller
- `怨种闺蜜` 繁中把官方「**保護**彼此最深的痛」寫成「把痛**藏**起來不讓對方知道」,方向相反
- `宝玉` 英文把「懷金悼玉」羅馬拼音化成 Huaijin Daoyu(那是形容詞不是劇名);
  「愛恨情仇」譯成 joy and sorrow
- `有一天我和祝英台去美术馆` 繁中刪掉官方開頭「**十二年前**…」,只剩「十二年後」,
  時間起算點消失
- `月亮与六便士` 英文把**一人分飾多角的演員分飾表**寫成劇情人物(繁中稿沒犯)

**錯在我自己的一項**:`拜风` 帳本寫「阿江/阿风 兩字合起來就是劇名『拜风』」——
實際是「江風」。英文稿忠實照抄成 `Their names together form Bai Feng`。
已更正帳本,並用正則掃過帳本全部 12 句斷言,確認只有這句是錯的。

**一項被推翻的懷疑**:本要把 `想把我唱给您听` 的 他/his 全部去性別化(官方確實沒交代),
查證後新浪娛樂報導明寫「小柯飾演她心懷愧疚的**兒子**」→ 原稿正確,不改。
查證層級記為【外部媒體佐證】,非 official_only。

### 新增 `scripts/zht_glyph_norm.py`(繁中字形/用語正規化)
慣例**從現有簡介庫數出來**,不憑印象:祕密 124:秘密 20、神祕 135:神秘 9、
舞台 522:舞臺 26、一齣 47:一出 8。
🚨 只做**逐詞白名單**不做整字替換——庫裡 8 個「一出」全是 一出生/一出口/一出現/唯一出口,
整字替換會改壞;秘魯/青瓦臺/臺中 同理保護。20 項正反向測試全過。
另補 `嶽州→岳州`(`流星之绊` 官方寫岳州府,OpenCC 轉成嶽州府;zh-tw 維基 岳州 23:嶽州 0),
並保護 五嶽/山嶽/嶽麓。
它**同時重跑台灣定譯表**,因為 `px_gen` 只在生成當下套用,補新詞對已產出的稿子不生效。

### `data/synopses_tw_terms.json` 補 阿爾茲海默→阿茲海默
來源:`想把我唱给您听` 官方寫「阿尔兹海默症」,逐字轉繁得到「阿爾茲海默症」,台灣不寫那個「爾」。
查證 zh.wikipedia zh-tw:阿茲海默 218 次 / 阿爾茲海默 10 次。

### `scratchpad/cn82/cn_gate.py` 四項強化(每項都做差異測試)
1. **拼音比對**:英文稿把中文名羅馬拼音化,原本一律標「無法比對」→ 12 組亮 10 組,等於沒有鑑別力。
   裝 pypinyin 後 `陈启耀→chenqiyao` 可真比對。對不上時**降級成「要人工看」不判錯**。
2. **拉丁名大小寫折疊**:官方寫全大寫(维塔 VITA),稿子寫 Vita,原本整批誤報「一個都沒出現」。
3. **反向檢查**:只在卡司表、官方劇情沒提到的名字跑進稿子裡 → 繁中 44 篇亮 5 盞、英文 38 篇亮 2 盞,
   **全是我先前人工判過的同一批**。
4. **標記外洩**:我標角色名的 `【】` 漏進 `心安东坡` 正文;另擋 🚨/⚠/「根據官方」。
5. **中英並列名**:官方給「Queen of Spades 黑桃女王」,中文稿卻寫英文那半
   (`嗜血博士`、`爱丽丝奇境之旅` 各中一次)→ 已同步修 `mk_regen` 的中文命名規則。

### 兩處靜默失敗改成出聲
- `scripts/polish.py` 台灣定譯表載入失敗原本 `except: pass`,`normalize_tw()` 會變成什麼都不做,
  呼叫端毫無跡象 → 改成 stderr 警告。
- `scripts/px_gen.py` 同一處的 `except Exception: pass` 同上。

### `scratchpad/cn82/apply_fix.py`(新增)
🚨 `px_gen` 還在跑時**絕不可寫回 `regen_*.json`**:它把整份 results 留在記憶體、每收一篇整檔覆寫,
中途寫進去的修正會被靜默吃掉(與 2026-09-04 那次 17 篇被覆蓋同一機制)。
所以修正先累積在 `fixes.json`,套用器會偵測 px_gen 行程並擋下(已實測 exit 2);
每條修正斷言原字串在該篇「剛好 N 次」,改不到當場失敗——實測擋下了 `Jonathan` 出現 2 次
卻被當單點修正的情況。

### 其他
- `docs/SYNOPSIS_SOP.md` 新增 §3.4:中國原創批次的八條規則。
- `.gitignore` 加 `scratchpad/*/_*.txt`(一次性診斷輸出,結論都已寫進 ledger/verify_log)。

---

## [v2.115.5] - 2026-09-04 13:09

> scratchpad 工作檔 + `px_gen.py` 續跑判準修正;**網站無變動**。重生成進行中,入庫 0 篇。

### 🚨 我造成的資料遺失:17 篇已查證的稿子被覆蓋

重啟繁中 job 後 `regen_zht.json` 從 19 筆掉回 2 筆。根因是**我自己的 bug**:

```python
# 續跑判斷式中文那半寫死了 400~460,沒跟著 LO/HI 走
if r.get('summary') and (LANG=='en' and LO<=size<=HI or LANG!='en' and 400<=size<=460)
```

我加 `PX_LO`/`PX_HI` 覆蓋時只改了**收稿條件**,漏了**續跑條件**。用 180~420 跑出來的合格稿多數低於 400,
續跑判定「未達標」→ 不計入 done → 整批重跑,而 `results` 是從空 list 重建、每筆都覆寫整個檔
→ 舊的 17 筆直接消失,**而且不報錯**。英文那半用的是 `LO/HI`,所以英文正常續跑(6→7)—— 對照組確認診斷。

已修成 `if r.get('summary') and LO <= r.get('size',0) <= HI`,並驗證舊判準會把 207 字的合格稿判成不合格。

**通則**:收稿判準與續跑判準**必須是同一個**,任一邊寫死常數就是靜默資料遺失。
同症狀掃描:px_gen 其餘常數是各語言預設值(會被正確覆蓋);`polish.py` 有兩處寫死 400–450,
但只影響檢視報告的長度標籤(OK/短/長),不影響資料,跨批共用故不動。

### 🚨 分頁洩漏:使用者提醒才發現有 19 個

原因不是並行,是**我用 `Stop-Process` 硬殺 px_gen** —— `ask_once` 有 `try/finally p.close()`,
正常結束不會漏,但整個行程被砍時 `finally` 來不及跑。本輪為了修 prompt 停了六七次就累積成 19 個。
新增 `close_tabs.py` 清到 4 個。規矩:改 prompt 前一次想完,真殺了就立刻清分頁
(清之前先確認沒有 job 在跑)。這也對應既有記憶「connect_over_cdp 卡死=前次崩潰遺留分頁」。

### 入庫前先挖出來的坑:`kb_merge` 一定要給 keymap

`kb_merge` 對每筆做 `key = keymap.get(title) or group_key(title)`,而我們 results 的 `show` 欄
**是整段重生成 prompt**。實測沒給 keymap 時算出的 key 是
`'0528 2026 28 0528 0528 eggy brandon doris doris 2026 09 04'` —— 簡介會**靜默掛在不存在的 group 上、
前端永遠不顯示、完全不報錯**。已寫 `mk_keymap.py`,並記下入庫後要回頭驗 `kb_merge.last.log`。

### `cn_gate` 大修:誤報 9/12 → 5/15,且通過負向測試

原本拿**整份卡司表**比對,但官方劇情多半不點名角色;更糟的是《亡灵之旅》帳本明令
**不可寫出「阿努比斯」**(官方刻意保留的身份懸念),稿子正確地沒寫卻被報成缺漏。
改成只比對**官方劇情裡真的出現過的名字**;官方寫「维塔 VITA」這種中英並列的,中文稿只寫中文也不再誤報。
**負向測試**:把《东南望》名字故意改錯,守門立刻報「缺 2/6 個官方角色名」✅。

### 查證進度:21 篇(ok 18、fix 3)

事實約束那一輪的產出**沒有一篇是編造情節**,3 筆 `fix` 都是譯名/角色列舉的一致性問題:

| 組別 | 問題 |
|---|---|
| 命运之上 zh-hant | 把四個卡司名塞成一句,官方劇情**完全沒點名角色**,會讓讀者以為多出四個人 |
| 嗜血博士 zh-hant | 用 Jonathan/Edward,但官方**劇情原文**寫「乔纳森/爱德华」,同篇應一致 |
| savior en | 「Lieutenant Piler」史實應為 Piller;「Professor Martin」漏姓 Bredius |

英文重跑後**零漢字**,而且文化詞處理正確:葵哥→Brother Kui(「哥」意譯不音譯)、
儺坪村/儺堂/儺面→Nuoping Village / Nuo Hall / Nuo mask、陈启耀→Chen Qiyao。

---

## [v2.115.4] - 2026-09-04 12:19

> scratchpad 工作檔 + `px_gen.py` 加字數參數;**網站無變動**。生成進行中,入庫 0 篇。

### 使用者裁定「中國批字數可降,內容正確優先」;連修四個會讓整批白跑的問題

#### `px_gen.py` 新增 `PX_LO` / `PX_HI` 環境變數

官方劇情原文多半只有 100~400 字梗概,而預設要求 400~450 字 = **逼 Perplexity 補它查不到的內容**。
中國批改成繁中 180~420 字、英文 110~300 **words**(⚠ 英文算字數、中文算字元數,單位不同)。

🚨 覆蓋字數時 **prompt 裡寫死的數字也必須一起換**,否則 prompt 還在叫它寫 450 字、程式卻用新區間收稿,
會變成每篇跑滿七次再取一個不合格的 best。用 `assert` 確保每個字串都真的被換到。
英文的數字**故意低於目標中心**(寫 250、目標 280,因為 Perplexity 一律超寫 25%),覆蓋時保持同一比例。
已驗**不設環境變數時預設完全不受影響**。

改完效果:繁中 191~291 字、英文 206~228 字,**全部第一次就達標**(原本要重試好幾次)。

#### 🚨 我判錯了一件事,已記進 `_overturned_suspicions`

我說《#0528》生成稿的「十三年前火災的鬼魂、來自紐奧良」是幻覺,還寫進上一版 CHANGELOG 當範例。
重生成時它又獨立寫出同樣細節並附具體資料,我回查 saoju.net:
**出品「吾哈文化传播(上海)」、製作「好好有戏」、首演 2023-11-18 環境式駐演、有鏡框版 —— 全部對上。**

精準地說:saoju 只佐證了**製作事實**,沒佐證那些情節細節。正確結論是**我的幻覺判定下得太早**,
不是「那些細節已證實」。教訓:官方梗概短 ≠ 多出來的都是編的,要先查再判。

順帶抓到帳本一個錯:《#0528》我寫「官方三個角色」**不完整** —— saoju 列出還有
【主宰/灵媒】【中介】【多功能人】,共 6 個角色欄。

#### 事實約束重生成(`mk_regen.py`):三個坑都是先單筆測出來的

| 問題 | 症狀 | 修法 |
|---|---|---|
| prompt 太長太結構化 | Perplexity 切成**研究報告模式** —— Markdown 表格、分節標題、巡演場次,1100~1400 字 | 壓成單行、任務講最前面、角色註記只取我標 🚨 的限制句 |
| 簡繁混雜 | 官方原文是簡體,繁中稿照抄成「维塔在稱量法庭…」 | 餵 prompt 前 **s2tw**(台灣字形、不轉詞彙;s2t 給異體字「爲」、s2twp 會改掉官方用語)。⚠ 只轉 prompt 副本,ledger 原文不動 |
| **設計衝突** | `has_summary()` 是**關鍵字比對不是語意判斷**:結尾寫「究竟意味著永生、重逢,還是更艱難的考驗?」文意完全對卻判 False → 每篇跑滿 7 次 | prompt 直接給可用收尾詞(全劇/主題/叩問/不只是),但明令不可帶入新事實 |

修完的單筆實測(《亡灵之旅：冥犬与少年》):

- 繁中 219 字、`has_summary=True`、專名全繁體、**懸念守住**(寫「一位狗頭人身、自稱『荷魯斯』的神」,
  沒點破他其實是阿努比斯 —— 那是官方刻意設計的身份欺瞞)
- 英文約 230 字,Book of the Dead / Court of Weighing / Field of Reeds **正確意譯而非音譯**

#### `cn_gate.py` 自己也有 bug:簡繁沒折疊

《savior》生成稿寫「范·米格**倫**」,帳本存的是簡體「范·米格**伦**」,比對落空 →
誤判成「官方角色名一個都沒出現 → 可能寫成別齣戲」。已用 OpenCC 折疊後比對,繁中稿不再整批誤報。

#### 查證管道實測(WebSearch 額度用完後改用瀏覽器)

- ✅ **豆瓣**音樂劇條目有完整劇情,而且條目的主創/劇院欄可以拿來跟官方圖逐項對照確認**是不是同一個製作**
  —— 《怪物》就是這樣脫離「無官方劇情」名單的(古家鹏/马心羽/李徐威/延幕文化/星空间18号 全中),
  順帶解開「兩人是什麼關係」的懸念(**兄弟**)並對上角色卡上的紅色 USA 戳章(美国康斯坦辛镇)。
  分級誠實標 **single**,豆瓣可編輯不是官方原文。
  ⚠ curl 403 要用瀏覽器,而且**連開第二頁就被擋**(整個 IP 被標記,不是單次限流)。
- ✅ **DuckDuckGo html 版**、**saoju.net** curl 可直接抓,後者有主創可對照製作身分。
- ✅ **搜狗微信搜尋**可列出公眾號文章,但中國宣傳推文正文幾乎都是圖,取不到劇情文字。
- ❌ Bing 會跳人機驗證(不解);百度/千岛/摩天轮/秀动都是 JS 渲染。
- ❌ 中国东方演艺集团官網網域已失效。

#### 10 組官方無劇情的現況

《怪物》已解決。其餘 9 組(ne zha、q、冈格尼尔、南墙计划、寻找李二狗、当你心动时、杜十娘、画中人、黑旗令)
大麥、保利、豆瓣、微信都查過了 —— **現階段拿不到可查證的劇情**。誠實結論:與其硬寫,不如維持沒有簡介。

---

## [v2.115.3] - 2026-09-04 11:16

> scratchpad 工作檔 + works.json 註記;**網站無實質變動**(只有 Borderline 的搜尋別名少一個裸字)。
> 簡介:**生成中(繁中 1/78、英文 3/78)**,入庫 0 篇。

### 冒煙測試就踩到同名劇陷阱;為全批加防頂替指示

#### 🚨 只給正向身份【擋不住】同名劇

《Borderline》第一次生成,Perplexity 寫出來的是**那部美國原創音樂喜劇**
(Anna Liddell、Dr. Brenner、Jessie、BPD 診斷)—— 即使 prompt 已經寫了
WELCOMEFUNNY / YUMIN / 缪时客。官方劇情其實是末世廢墟的暗殺組織,角色叫 July / June / February。

加上**反向消歧義**(明說「不是 Benjamin Velez 作曲那部同名美國音樂喜劇,也不是任何 BPD 題材作品」)
之後,第二次就寫對了。SOP §1 本來就允許這種寫法(Pinocchio 的「不是迪士尼版也不是原著」),
而且只說「不是什麼」不洩漏本劇劇情,不影響查證獨立性。

→ 已為**全部 78 組**加上統一的防頂替指示:「只描述上面這一部製作;找不到就直說找不到,
不要用同名或題材相近的其他作品代替,也不要憑劇名推測」。這批劇名撞名作的太多
(牡丹亭、海上钢琴师、边城、花木兰、荒原、青春之歌、杜十娘、莫比乌斯、奇美拉…)。

#### 🚨 資料真空時會長出整套細節

《#0528》官方只寫「曾經發生過一次火災」,生成稿長出「十三年前火災死去的鬼魂、來自紐奧良、
試鏡失敗」,並把官方明訂只用英文的 Eggy/Brandon/Doris 自創成艾吉/朵莉絲/布蘭登。

#### 新增 `cn_gate.py`(生成後第一層機械把關)

三種檢查,全部來自冒煙測試實際踩到的錯:

| 檢查 | 抓什麼 |
|---|---|
| A | 官方角色名**一個都沒出現** → 很可能寫成別齣戲 |
| B | 官方角色名有英文寫法、生成稿整篇找不到 → 被中譯了 |
| C | 帳本明寫「不可寫成 X」的詞出現了 |

拿兩份已知有錯的稿驗過,**兩種錯都抓到而且能區分**:
v1(寫錯戲)報「角色名一個都沒出現」;v2(戲對、名字譯錯)報「缺 3/4 + 官方英文名疑被中譯 +
禁忌詞 二月/六月」—— 與我人工判讀的結論一致。

抽取器修了四輪才夠乾淨:【】在帳本裡不只標角色名也標說明詞(「狗头人身」「英文月份」),
禁忌詞正則原本會夾到卡司名,整份名單塞在同一個【】裡時又會被長度限制整段濾掉
(《风声》8 個角色只抽到 2 個)。

#### works.json:Borderline 的同名風險【如實記為未解】

拿掉了裸的 `"Borderline"` 別名,但 canonical `Borderline-边界线` 正規化後本來就是 `borderline`,
**那部美國劇一旦進目錄仍會被吃成韓國原創**。works.json 沒有 not_musical.json 那種場地限定機制。
已寫明偵測徵兆(出現 tag=韓國原創 但場館在英美的 borderline 場次)與屆時的修法,不假裝修好。

#### 10 組官方無劇情:第二來源也查過了

保利票務(`/good/search-products-data` → `/good/project/detail/{id}`)全部試過:
杜十娘、南墙计划 有頁面但只有購票須知;寻找李二狗、当你心动时、怪物、冈格尼尔、画中人、黑旗令
在保利沒有頁面;阿Q与吴妈、哪吒 命中的全是**同名的別齣戲**(阿加莎、哪吒闹海兒童劇、皮影戲)。
→ 這 10 組的官方劇情確實不在任何售票平台,查證要比其他組更嚴,查不到就誠實標 internal。

---

## [v2.115.2] - 2026-09-04 10:48

> 只加 scratchpad 工作檔與守門腳本,**網站沒有變動**;簡介仍是 0 篇生成、0 篇入庫。

### 中國批階段 2 備料:78 組身份提示 + 兩道生成前守門

`identity.json` —— 78 組 prompt **本人逐條寫好**(出品/主創/場館/年份),
`list.json` + `order.json` 已備妥(有官方劇情的 68 組排前面,查證關卡最硬的先跑)。

#### 🚨 守門當場抓到我自己三個違規

SOP §1 明文「prompt 只釘身份,絕不餵劇情」——餵了之後 Perplexity 只是把我的字吐回來,
我再拿它「查證」就是球員兼裁判。我寫 identity 時還是犯了:

| 組別 | 我原本寫的 | 問題 |
|---|---|---|
| 命运之上 | 「角色有 JOHNNY、SHIRLEY、路西法、拉斐尔」 | 整份官方角色表 |
| 她的海 | 「女海盗石香姑题材」 | 主角姓名 |
| 玉良 | 「潘玉良题材」 | 傳主姓名(劇名只有《玉良》) |
| 月亮与六便士 | 主演名單 | 其中「威廉」與角色名撞名 |

四筆全部改掉。`玉良` 改用【製作顧問 安徽博物院】+ 香港主創名單釘身份 —— 這樣
Perplexity 若寫出傳主生平,那是它自己查到的,§3 查證才不是自問自答。

#### 新增 `leak_check.py`(生成前必跑)

1. identity 含官方角色名(**劇名本身含的不算** —— 《阿Q与吴妈》《宝玉》避不掉也不需避)
2. identity 與 `official_plot` **連續相同 ≥12 字**(比對前先剝劇名,否則官方劇情多半以
   「音乐剧《X》…」開頭,每組都誤報 = 狼來了)
3. 例外必須寫在 `leak_ok` + `leak_ok_why`,**不是靜默忽略**(目前只有《金童子》的
   `Martin Booth` / `Gweilo`,那是原著作者與書名,SOP 允許用授權來源釘身份)

已做差值測試:抄一段官方劇情(17 字)被抓、乾淨身份句(1 字)與短片段(6 字)放行;
改良角色名抽取器後**重跑回歸**,確認仍抓得到真洩漏。

#### 其他新增

- `sync_check.py` —— 帳本↔目錄對帳(目錄有而帳本沒有 = 0;帳本有而目錄沒有的必須有 `catalog_status`)
- `mk_identity.py` —— 從帳本抽身份草稿,**只讀 note,絕不碰 `official_plot`/`characters`**
- `poll_deploy.py` —— 部署輪詢,標記必須本次獨有(先驗正式站 0 匹配才開始等)

#### ⚠ 生成尚未開始

Comet 在跑但沒開 `--remote-debugging-port=9223`,而這個參數**無法對已啟動的行程補掛**,
要用生成管線就得重啟瀏覽器(會關掉使用者當下開著的視窗)。等使用者決定,不自行動手。

---

## [v2.115.1] - 2026-09-04 10:18

> 網站有變動(數字基於同日 CI cron `a04e2e48` 重跑全部 scraper 後的版本,不是我先前的本機基準):
> shows **2084→2081**(−3=兩齣非音樂劇的三個場次)、catalog group **623→621**、
> 中國原創 142→137 場次、韓國原創 148→150、served **528**(修好庫鍵前會是 527)、DATA_VER 更新。

### 中國批分類裁決落地;順手救回一篇被鍵打錯埋掉的簡介

#### 分類處置三件

| 項目 | 處置 | 落地 |
|---|---|---|
| 《男孩、鼹鼠、狐狸和马》《海底总动员》 | 非音樂劇,踢掉 | `not_musical.json` +2(-3 場次 / -2 group) |
| 《金童子 Gweilo》《边城 BORDER TOWN》(香港製作) | 仍留中國原創,帳本註記港製 | 只改帳本,分類不動 |
| 《哪吒 NE ZHA》(倫敦首演) | 維持中國原創 | 不動 |

> ⚠ 前兩齣我拿去問了使用者,被指正「非音樂劇本來就該踢掉不是嗎」——**對**。`not_musical.json`
> 早就定義了證據標準(依製作方官方文案,平台分類欄不算),NARUTO 也有前例,這是**套用既定規則**
> 不是需要裁決的新政策。真正需要問的只有沒前例的分類法問題(香港作品要不要開新分類)。
> 已寫進 `docs/SYNOPSIS_SOP.md` §0.5。

另兩組**確定標錯**的照改:`桑塔露琪亚` 與 `Borderline-边界线` → **韓國原創**(`works.json` 註冊,
`_origin_note` 記證據)。中國原創 144→137 場次 / 82→76 group,韓國原創 148→150。

#### 🚨 順手抓到:一篇簡介因為庫鍵打錯,永遠送不出去

對帳時發現 served 528 與庫 605 的落差裡藏了一筆**不該落在裡面的**:
...
简体「钟」還原繁體時 **鐘/鍾 二選一選錯**。這齣果陀劇場的台灣劇(苗北藝文中心,11/13,**檔期還沒到**)
三語簡介早就備好,卻一直沒被服務 —— 而管線只印「庫 605 → served 527」,**完全不會出聲**。
已用 OPENTIX 官方頁(`<title>` 為「果陀劇場音樂劇《生命最美好的5分鐘》2026苗北藝術節」)確認正字為
「分**鐘**」,三語庫鍵全部更名,served +1。

**加了守門**:`build_served_synopses.py` 現在會對「與某個 catalog group 極接近但不相等」的庫鍵發
`::warning::`。⚠ 只告警不自動合併 —— 同一輪掃描也撈到 `テニスの王子様` vs `新テニスの王子様`,
那是**兩部不同作品**,合併就錯了。守門已做差值測試(注入舊錯誤鍵會抓到;正確鍵/無關鍵/空清單都靜默)。

#### 🚨 帳本自身也有兩對重複條目

同一齣戲被建了兩次,因為不同輪次分別用了【目錄 group key】與【劇名】當鍵:
`q` vs `阿Q与吴妈`、`在那遥远的地方2 0` vs `在那遥远的地方2.0`。
**純字串正規化的查重抓不到這種**(`q` 與 `阿q与吴妈` 正規化後完全不同)。已合併,往後一律以
`shows.json` 的 group 當帳本鍵,並新增 `sync_check.py` 拿目錄對帳。

合併 `阿Q与吴妈` 時發現兩份條目**對「有沒有官方劇情」判斷相反** —— 舊條目把官方的定位宣傳文字
(「依托…改编创作」「以正气庄重的舞台形式演绎」)當成 `official_plot` 收了。已採嚴格判定:那段沒有情節,
不算劇情簡介,status 維持 `partial`。

#### ⚠ 另外兩組消失與我無關(已用 A/B 確認)

`不负青山`、`在那遥远的地方2.0` 的 `end_date` 都是 2026-08-31,今天 09-04 被「已結束檔期」規則
正常剔除。把改動 stash 掉重建 baseline 也一樣掉,我的改動剛好等於 −3 場次。

#### ⚠ 與 CI cron 撞車的處理

第一次 push 被拒(non-fast-forward):同日 CI `a04e2e48` 剛重跑全部 scraper 並重建網站。
我本機的產出檔是基於**舊 scraper 資料**建的,直接 rebase 會把 CI 的新資料蓋掉。
處理方式:只保留我的**源頭檔**編輯(`not_musical.json` / `works.json` / `synopses_library/*` /
`build_served_synopses.py` / 文件),`reset --hard origin/main` 後重新套用,再在 CI 的新資料上
**完整重跑管線**。最終差值確認乾淨:消失的 group 只有那兩齣非音樂劇,新增 0。

#### 帳本現況

82 組(去重後)· 需要簡介 78 組 · 已下架不需生成 4 組 · 對帳 `目錄有而帳本沒有 = 0`。
**簡介仍是 0 篇生成、0 篇入庫**,階段 2 尚未開始。

---

## [v2.115.0] - 2026-09-04 07:42

> ⚠ 只進帳本,**沒有簡介入庫、網站沒有變動**(覆蓋率仍 534/620)。

### 中國批帳本 84/84 建檔完成;抓取管線抓出三個同型的「靜默截斷」

#### 🚨 主因:`harvest.py` 的 `rec["imgs"][:6]` 靜默截斷

40 組 damai 詳情頁裡 **35 組的官方長圖超過 6 張**,共**漏抓 167 張**。截斷不報錯,
log 印「圖6」看起來完全正常。其中 **26 組當時已經建帳**,等於帳是拿不完整的素材建的。

寫了 `topup.py` 補齊 + `newsheet.py` 把 167 張補圖拼成接觸表逐張回看。**實質損害:**

| 組別 | 帳本原本寫的 | 實際 |
|---|---|---|
| 命运之上 | 「天使與惡魔官方未列為角色卡上的角色」 | **錯**,〈路西法〉〈拉斐尔〉兩張正式角色卡就在被截掉的圖裡 |
| 新贵 | 「兄妹的名字官方沒給,不可自創」 | **錯**,〈张衡〉〈阿旭〉〈黛安娜〉三張角色卡被截掉;另查出原著是科幻作家**鲁般**(星雲獎金獎) |
| 莫比乌斯 | 「只有路易斯一個角色標了名字」 | **錯**,〈戴蒙 Damon〉〈欧文 Owen〉被截掉 |
| 夜幕下的哈尔滨 | 「官方無劇情,status=partial」 | **錯**,完整劇情簡介三張全被截掉;角色從 2 個補到 **11 個** |
| 杜十娘 | 角色 5 個 | 補到 **11 個** |
| 大江东去 | 「其餘角色官方未列」 | 補到 **9 個**角色欄 |
| 蓝关雪 | 「角色名待確認」 | 補到 **7 個**角色欄 |
| 海浪你轻轻地摇 | 角色名 `孙丽丽` | **誤植**,官方海報寫 `孙丽娟`;另漏一個官方角色〈李莉〉 |

#### 🚨 同型錯誤之二:`[:6]` 之外還有「不滿 6 張就不檢查」的假設

第一輪回掃只掃「已下載滿 6 張」的組,假設「不滿 6 張 = 來源就這麼多」——**那個假設沒驗過**。
改成對所有 damai 詳情頁核對來源總數後,又抓到 8 組被截斷。

#### 🚨 同型錯誤之三:每組只取「第一筆演出」的 ticket_url

`男孩、鼹鼠、狐狸和马` 原本完全抓不到素材,因為第一筆(珠海場)給的是**大麥搜尋頁**;
同組廈門場其實有正常詳情頁。

#### 產地標錯(帳本已記 `tag_error`,尚未改 code)

| 組別 | 實際產地 | 證據 |
|---|---|---|
| Borderline-边界线 | 🇰🇷 韓國 | 原版製作 WELCOMEFUNNY、編劇 YUMIN、作曲 PARK JIYOUNG、有韓語校對 |
| 哪吒 NE ZHA | 🇬🇧 倫敦西區 | 官方引新華社倫敦電:**全球首演在倫敦蕭劇院**;導演 Jennifer Tang;全英文演出配中文字幕 |
| 金童子 Gweilo | 🇭🇰 香港 | 一條褲製作(胡海輝);英語及粵語演出 |
| 边城 BORDER TOWN | 🇭🇰 香港 | 官方寫「**當代中國香港粵語音樂劇扛鼎之作**」「香港中英劇團攜手北京鼓樓西戲劇」;本輪標明是**內地首演** |
| 桑塔露琪亚 | 🇰🇷 韓國 | (先前已記) |

另有 5 組產地待判定(`tag_check_needed`):流星之绊、玉良、有一天我和祝英台去美術館(非常林奕華·港台陸合製)、
男孩鼹鼠狐狸和马(英國繪本改編)、海底总动员。

#### ⚠ 兩組可能不該留在音樂劇目錄裡

- `男孩、鼹鼠、狐狸和马` — 官方**自稱「沉浸式舞台劇」,全文沒出現「音樂劇」**,形式是人偶+肢體+配樂
- `海底总动员` — 官方自稱「大型童話音樂劇」但同頁又寫「這部**舞台劇**」;深圳兒童劇團布偶裝演出

`海底总动员` 另需注意:**與皮克斯《Finding Nemo》只是同名**,劇情是母親救孩子(電影是父親找兒子),
絕不可合併或標為其改編。

#### 破解 JS 渲染來源

- **shcstheatre**(宝玉/边城)節目內容是非同步塞進 DOM 的 —— 先前抓到「一字不差的 4781 字」就是抓到全站共用版面。改打後端 `/webapi.ashx?op=GettblprogramCache`。
- **juooo**(4 組)純前端渲染 → Playwright。
- **poly**(3 組)不在大麥上架;`/good/product-info/{id}` 是陷阱(**會忽略 id**,固定回傳瀋陽的另一齣戲)。正確端點 `/good/project/detail/{id}` 是把 H5 的 JS bundle 裡所有 `/good/...` 字串掃出來找到的。

#### 帳本現況

84 組全部建檔 · 74 組有官方劇情 · 10 組官方確實沒有劇情(status=partial) ·
43 組有英文劇名 · 41 組**查證後確認官方沒有英文劇名**(不是沒查)。

#### 新增工具

`topup.py`(補截斷圖)、`mk_meta.py`(重建圖片索引,raw.json 一變就必須重跑)、
`sheet.py`(接觸表定位)、`newsheet.py`(跨組補圖接觸表)、`fetch_js.py`、`fetch_shcs.py`、
`poly_detail.py`、`damai_search.py`、`dl_js_imgs.py`。

> 🚨 接觸表**只能定位不能讀字**:凡是從縮圖直接抄的人名,本輪**無一正確**
> (新贵 玄影/阿祖/莫安娜 → 實為 张衡/阿旭/黛安娜;怨种闺蜜 林芜/何梦晨/林薇 → 實為 林霄/钟星盈/林霖)。

---

## [v2.114.4] - 2026-09-04 01:39

> ⚠ 只進帳本,**沒有簡介入庫、網站沒有變動**(覆蓋率仍 534/620)。

### 中國批讀圖 47/82;我自己立的通則被推翻兩次

#### 🚨 通則一被推翻:「大麥劇情全在長圖裡」是錯的

至少 **16 組的劇情在頁面文字(itemExtend)裡**,不在圖裡。其中《大江东去》《日出渤海耀天山》
**連「劇情簡介」標題都沒有**、文字本身就是劇情,所以我第一次用標題關鍵字掃描時整批漏掉。
`羊角尖` 更是先前被我歸為「抓不到圖」的組 —— 它不是抓取失敗,是**根本沒有詳情圖**。

#### 🚨 通則二:我改良後的掃描又產生假陽性

第二版掃描判定 `宝玉`、`边城` 各有 4174 字劇情 —— 實際是 shcstheatre 的**網站框架文字**
(導覽選單/租場服務/劇院導覽)。**破綻是兩組長度完全相同**。加上跨組去重後才排除,
同時抓出 `狐说臣与仙` 與 `破墙` 共用另一段 318 字樣板。

#### 英文劇名:感謝提醒,已升格為必填欄位

先前我只在看到時順手記,沒有系統性查。盤點 28 組時發現 **`牡丹亭` 的 `THE PEONY PAVILION` 漏掉了**
—— 因為我只讀劇情簡介那張,沒看主視覺海報。已把 `title_en` 改為獨立欄位,值只有三種:
有英文名 / **已查證確認沒有** / 待查。目前 24 組有英文劇名。

回頭看海報時還**救回一個我判錯的限制**:`三风一树` 我寫了「🚨 只有三個角色,不可寫出第四人」,
但海報明寫【三位演員分飾多角】。我的「限制」本身是錯的,已修正並記進 `_overturned_suspicions`。
...
|---|---|---|
| `狂飙` | 黑勢力**之王** | 黑勢力**頭目** |
| `风声` | 超 **75 萬**名觀眾 | 超 **7.5 萬**名觀眾 |
| `新唐伯虎点秋香` | 華**人娘**夫人 | 華**太師**夫人 |
| `蝶变` | **墨**園路 | **愚**園路(上海真實路名) |
| `savior` | 當我**索自**揭開 | 當我**親手**揭開 |
| `savior` | 承受**其**相的代價 | 承受**真**相的代價 |

#### 這一輪新增的「不可自行補」案例

- `海浪你轻轻地摇` —— 卡司卡有「譚嘉薇 飾 **孫麗麗**」,但內容介紹**完全沒有這個角色**;
  從「妻子擺攤」推測她是王大焊的妻子很順,但官方沒說 → 不認定。
- `savior` —— 台詞提到【維米爾】,但官方**沒說那幅「國寶」是哪幅畫**;納粹【戈林】是劇情提到的買家,
  **不在角色表上**,不可寫成登場人物。
- `灯塔` —— 官方說霍普受「朋友」影響,角色表剛好有第三人懷恩,但官方沒說是他。

#### 同一人兩個名字的陷阱(已遇三次)

`昨日显影` 的【安妮 = 任安然】、`日出渤海耀天山` 的【梅二月 = 梅雙全】、
`钱氏今生` 的錢鏐(易誤寫成錢謬)。前兩者若照字面拆開就會多寫出一個角色。

#### 產地判定累積

- ❌ `桑塔露琪亚` —— 韓國引進(編劇 Lee Hee Joon/作曲 Park Hyun Suk),標籤要改
- ⚠ `流星之绊` —— 有講談社授權標誌,待查
- ⚠ `玉良` —— 主創幾乎全是香港班底(演戲家族),待查
- ✅ `蝶变` —— 官方明打【版權輸出韓國】,是中國原創**外銷**
- ✅ `海上钢琴师` —— 官方明寫【中文原創】,雖有法國作曲家 Dove Attia 等外籍主創
- ✅ `时光代理人` —— 改編自中國自己的 bilibili 動畫 IP

## [v2.114.3] - 2026-09-04 00:50

> ⚠ 只進帳本,**沒有簡介入庫、網站沒有變動**(覆蓋率仍 534/620)。

### 中國批讀圖進度 28/82,累積 55 條「不可自行補」的硬性限制

大麥 59 組已讀 28 組(完整 24、缺官方劇情 4)。這批帳本最大的價值不是劇情本身,
而是**逐條記下「官方沒說、所以不准寫」的地方** —— 目前 55 條。抽幾個代表:

| 劇 | 官方沒說什麼 | 為何容易寫錯 |
|---|---|---|
| `灯塔` | 影響霍普的那位「朋友」是誰 | 角色表剛好有第三人「懷恩」,連起來很順但無依據 |
| `嗜血博士` | 瑪莎的身分;吸血鬼是否真實 | 官方刻意留白「究竟什麼是真實,什麼是虛幻?」 |
| `命运之上` | JOHNNY/SHIRLEY 各對應哪個身分 | 從卡司照片性別推得出來,但那是推論 |
| `新贵` | 那對兄妹的名字 | 只有李K有名字 |
| `荒原` | 許夕燃入獄的原因 | 官方只寫「出獄歸來」 |
| `去你的夏天` | 小松與小偉的真實身分 | 官方稱「兩個身份成謎的怪人」,是懸念 |
| `莫比乌斯` | 路易斯以外的角色名 | 卡司頁沒有角色標題 |

#### 新型態陷阱:沉浸式宣傳道具 ≠ 劇情

`新贵` 的物料裡有一封以劇中組織【PDO】名義寫給「市民朋友們」的公告
(內容:新貴計劃即將結束、10 月 31 日後不再受理申請)。那是**虛構世界觀的宣傳道具**,
旁邊還真的有一張**真實的演出取消公告**,兩者版面相似。不讀清楚會把道具當劇情寫。

#### 其他這一輪確立的規則

- **「聲音出演」是配音,不是台上角色**(`三风一树`、`荒原` 各中一次)
- **重製版不可套原典**:`杜十娘`(劇本打磨/曲目新增)、`牡丹亭`(官方明說改動了原劇結構,
  把平行蒙太奇改成兩線編織)
- **改編作品的角色名要照本製作**:`爱丽丝奇境之旅` 官方是 Queen of Spades(不是原著紅心女王)、
  Clark Hare(不是三月兔)
- **官方只有英文名時不可自創中文**:`命运之上` 的 JOHNNY/SHIRLEY

#### 工具改良

新增 `zoom.py`:把長圖切成固定高度的連續段落並統一放大到寬 1400。
在此之前裁切範圍要靠猜,《昨日显影》裁了三次才讀完(先裁到標題、再寬度不足切掉右邊)。

## [v2.114.2] - 2026-09-04 00:22

### 🚨 我把 219 個衍生商業圖(72.9 MB)推進了 public repo,已從追蹤移除

**這是我的錯,如實記錄。**

v2.114.1 那次我用 `git add scratchpad/cn82/` 一次加整個目錄,而當時 `.gitignore` **只擋了
`scratchpad/cn82/img/`(原圖)**,沒擋我自己後來產生的四個衍生圖資料夾:

| 資料夾 | 用途 | 檔數 |
|---|---|---|
| `sheet/` | 接觸表(每組所有圖縮成一張並排) | 59 |
| `crop/` | 裁切放大的劇情區 | 57 |
| `seg/` | 長圖分段 | 114 |
| `zoom/` | 分段放大(後來新增) | — |

合計 **219 檔、72.9 MB**,全部是大麥官方詳情圖的再製品,同樣不可散布。

**為什麼會漏**:我在收割完成後有主動檢查 gitignore,但只想到「原圖」,沒想到**自己接下來會產生
衍生圖**。擋 `img/` 的當下,那四個資料夾還不存在。

**已做**:`git rm -r --cached` 移除 230 檔追蹤,並把四個資料夾補進 `.gitignore`。

**尚未做**:這些檔案**仍在 git 歷史裡**。要真正清除需要改寫歷史(filter-repo + force push),
那是破壞性操作且會影響所有 clone,**我不會自行執行,等使用者決定**。

**順帶修正一個我自己的誤判**:我一度用 `git check-ignore scratchpad/cn82/sheet/` (資料夾路徑)
測試,得到「未忽略」而以為規則沒生效;改用實際檔案路徑 `.../sheet/x.jpg` 測才發現規則是對的。
**測 gitignore 要用檔案路徑,不能用資料夾路徑。**

### 讀圖工具改良

讀到第 22 組時發現裁切範圍常抓不準(《昨日显影》裁了三次:先裁到標題、再寬度不足切掉右邊、
第三次才完整),因為各家排版的文字塊寬度位置都不同。新增 `zoom.py`:把長圖切成固定高度的
連續段落並統一放大到寬 1400,照順序讀即可,不必每次猜範圍。

### 進度

帳本 23/82(完整 19、部分 4)。生成 0 篇,未入庫,網站無變動(覆蓋率仍 534/620)。

## [v2.114.1] - 2026-09-03 23:56

> ⚠ 這一版**只進工作檔**(中國批的收割器、帳本、讀圖工具),**沒有任何簡介入庫、網站沒有變動**。
> 覆蓋率仍是 534/620。

### 中國原創 82 組開工:官方劇情【全在長圖裡】,只能本人逐張看圖

全站劇情缺口 86 組裡有 **82 組是中國原創**(佔 95%)。開工後查清楚素材在哪:

| 來源 | 組數 | 官方詳情在哪 |
|---|---|---|
| 大麥 damai | 60 | 內嵌 JSON 的 `itemExtendInfo.itemExtend`,**文字只有購票須知,劇情全在長圖** |
| polyt.cn | 16 | JS 渲染,通用抓法只拿到網站框架文字 |
| juooo | 4 | 同上 |
| shcstheatre | 2 | 同上 |

大麥與 Interpark 是**同一個模式**:官方 SYNOPSIS 做成長圖。已收割 **375 張圖、307 MB**。

#### 🚨 307MB 商業圖原本沒有被 gitignore

`scratchpad/cn82/img/` 一開始**不在** `.gitignore` 裡。已補上並確認 `git status` 中圖檔數為 0。
(這是踩過的坑:曾一次誤入 803 檔 387MB 進 public repo。)

#### 讀圖工法:接觸表定位 → 裁切放大 → 逐字讀

試過三種,前兩種不夠用:
1. ~~OCR 索引~~ —— pytesseract 有但 tesseract 執行檔沒裝
2. ~~長寬比挑最高那張~~ —— 38 組命中,但《女巫》最高那張其實是卡司表,劇情在另一張
3. ✅ **接觸表**(每組所有圖縮成一張並排)—— 一眼看出哪張有「劇情簡介/故事梗概」

**放大那一步不能省,今天已靠它救回三次轉錄錯誤**:

| 劇 | 我在縮圖上讀成 | 官方實際 |
|---|---|---|
| `狂飙` | 黑勢力**之王** | 黑勢力**頭目** |
| `风声` | 超 **75 萬**名觀眾 | 超 **7.5 萬**名觀眾(差十倍) |
| `新唐伯虎点秋香` | 華**人娘**夫人 | 華**太師**夫人 |

#### 🚨 產地標籤不可信,已抓到誤標

- `桑塔露琪亚` —— 被標【中國原創】,實為**韓國引進**。百度百科:編劇 Hee Joon Lee、作曲
  Hyun Suk Park,中文版劇本由金琪、趙陽改編;saoju.net 直接列「引进/原版韩国」。**標籤要改**。
- `流星之绊` —— 官方標「中文原创」,但主視覺有【KODANSHA 講談社】標誌,情節與東野圭吾同名小說
  一致(流星夜滅門、三兄妹詐騙為生、獵物是富家公子、意外發現真兇),應為**取得日方授權的在地化改編**
  (場景改為岳州府平江縣、姓氏改李)。標為待查,不逕自沿用。
- 對照組 `海上钢琴师` —— 同樣有大量外籍主創(法國作曲 Dove Attia、西班牙舞美 Alfons Flores),
  但官方明寫【中文原创音乐剧】→ **有外籍主創 ≠ 引進劇**,標籤成立。

#### 角色表再次證明是最有效的關卡

- `爱丽丝奇境之旅` —— 掛著《愛麗絲夢遊仙境》的名字,但官方角色是 **Queen of Spades 黑桃女王**
  (不是原著的 Queen of Hearts)、**Clark Hare 時鐘兔**(不是 March Hare);劇情是地心世界、
  光源劍、光源日,原著一個都沒有。不讀圖就一定寫錯。
- `辛吉路的画材店` —— 三個角色裡,**辛吉開場前已因病去世**、**塞修是漫畫裡穿越出來的人物**。
- `当你心动时` —— **小白是「心動的化身」**,不是人類角色。
- `三风一树` —— 「聲音出演」那 6 人是**配音**,不是台上角色。

#### 進度

帳本 19/82(完整 15、部分 4)。缺官方劇情 4 組、產地待查 2 組、角色名待確認 1 組,全部明確標記,
不會被當成已確認事實使用。生成 0 篇,未入庫。

## [v2.114.0] - 2026-09-03 18:02

### 歐陸/日台/百老匯 19 組三語簡介入庫 —— 五個分類補到 100%

劇情覆蓋 **515/620 → 534/620 (83.1% → 86.1%)**。除了中國原創(82 組整批未做),
現在只剩 **4 齣**沒有簡介 —— 就是那 4 齣官方完全沒公開劇情的韓國戲。

| 分類 | 今天早上 | 現在 |
|---|---|---|
| Broadway/West End | 232/238 | **236/236 = 100%** |
| 歐陸原創 | 52/63 | **63/63 = 100%** |
| 西葡音樂劇 | 55/58 | **56/56 = 100%** |
| 台灣原創 | 42/43 | **43/43 = 100%** |
| 日本原創 | 26/29 | **28/28 = 100%** |
| 韓國原創 | 86/90 | 86/90(4 齣官方零資訊) |

#### 只有【逐篇通讀】抓得到的錯,機械把關一個都抓不到

這批最後一輪人工通讀簡中 19 篇,抓出六類實質錯誤:

| 劇 | 錯誤 | 性質 |
|---|---|---|
| `Blood Brothers` | 「莱昂斯太太开枪击中米奇,警察随即射杀爱德华」—— **開槍的人寫反了**(正確是米奇殺愛德華、警察再擊斃米奇) | 情節硬錯 |
| `Carrie` | 體育老師寫成「德贾尔丹」= 原著小說/電影的 **Miss Desjardin**,音樂劇版是 **Gardner** | 把原著人物寫成本製作角色 |
| `On Your Feet!` | 「父亲**因**参加猪湾事件**而**长期病痛缠身」—— 參加屬實,但多發性硬化症與豬灣無因果關係 | 因果造假 |
| `Piotruś Pan` | 把迷失男孩 Powtórka/Ciutek/Trąbal/Rybol/Szpic 寫成**虎克的海盜**;虎克失去的是手不是手臂 | 陣營錯置 |
| `Romeo i Julia` | 加了官方從未提及的**維羅納**(本作是有毒販角色的現代改編) | 套用通用版設定 |
| `Na prochach` | `Konferansjerka`(女司儀,**普通名詞**)被音譯成「康费朗谢尔卡」 | 音譯普通名詞 |

#### 五輪生成,每一輪都在修上一輪【自己帶進來】的問題

1. **第一輪**(19 齣,只釘身份不給情節):讓產出獨立於帳本,查證才不是球員兼裁判
2. **第二輪**(8 齣,餵官方事實):原問題全修好,但**開始列名單**——把
   「Carole/Konferansjerka/Brenda」這種角色欄位連斜線抄進正文;`天堂邊緣` 英文稿夾中文字;
   `プリキュア` 英文 **7 次全部回傳空白**(提示幾乎全是否定句)
3. **第三輪**(3 齣):修中文批才抓到的 `Starzyński` 1941/1939 自相矛盾、`NARUTO` 整篇漫畫劇情、
   `Charlie Brown` 的「學校」與漏掉 Linus 演說
4. **第四輪**:修列名單與夾中文字。但我寫的「日文漢字名必須照抄」**英文版也照做**,整篇夾漢字
5. **第五輪**:修 `Starzyński` 英文又犯的列名單、簡中 `Piotruś Pan` 的海盜錯置。
   後者修好海盜卻**把弟弟名退回 John/Michael**(官方是 Jaś/Miś)

**「修 A 引入 B」在這批發生四次**,其中三次是我的提示造成的。教訓已寫進 SOP §3.3。

#### 手動修正層

單一詞句的錯不值得再跑生成,但直接改 `merged_*.json` 會被重跑靜默蓋掉(今早才踩過)。
新增 `scratchpad/gap19/fixes.json`,由合併腳本在所有重生成之後套用,**每一筆都要在 `_why`
寫理由**。目前 26 筆,涵蓋 8 齣。

#### 入庫時抓到我自己造成的 key 錯誤

`プリキュア` 的 group key 我在建帳本時自己編成 `precure colorful runway`,
但目錄裡是 **`カラフルランウェイ プリキュア`** —— 對不上就掛不到卡片,19 組只進了 18 組。
已改正 key、清掉孤兒條目,並同步更正 `order.json` 避免重跑再錯。

#### 查證與被推翻的懷疑

外部查證 9 齣(EDDA 由匈牙利維基逐句證實、Két összeillő ember 的 1972 廣播相遇與兩人死因、
Blood Brothers 由 El País 加泰語劇評證實沿用 Mickey/Eddie、Kajko i Kokosz 由 Polityka 證實
以漫畫專輯為本)。

**我判錯四次**,全部記在 `scratchpad/gap19/verify_en.json` 的 `_overturned_suspicions`:
Bitwa o tron 的三個角色(我抽表時截斷)、Szécsi Pál 的自殺(維基寫明)、
Łamignat 的「強盜」(波蘭維基寫明他既是雅加的丈夫也是強盜)、Tania Ruzs 的日期。
這四次若照直覺改下去,就會把正確的內容刪成錯的。

## [v2.113.4] - 2026-09-03 15:38

> ⚠ 這一版**只進工作檔**(帳本與查證腳本),**沒有任何簡介入庫、網站沒有變動**。
> 覆蓋率仍是 515/620。留這筆是為了讓查證過程可追溯。

### 歐陸/日台/百老匯 19 組缺口:官方原文與官方角色表建檔完成,三語生成完成,英文批通讀完畢

#### 官方角色表是這批最有效的關卡

先建帳本(19/19 官方原文 + 19/19 官方角色表)再生成,擋下的錯:

- `Na prochach` —— 台上家族姓氏是**虛構的 Barker**,不是現實中的 Sackler。
  官方文案只說「被稱為美國最糟糕的家族」「一家製藥公司的老闆」,全篇沒有 Sackler 或 Purdue。
  沒有角色表就一定會寫錯。
- `Romeo i Julia` —— 官方角色表有莎劇沒有的 **Dealer(毒販)**,內容警告列毒品/自殺/殺人,
  且**沒有蒙太古家長** → 是現代改編,不能套通用莎劇。
- `Piotruś Pan` —— 有 **Dorosła Wendy(成年溫蒂)** 與 **Jane(她女兒)**,含長大後的框架。
- `Két összeillő ember` —— **只有兩個角色**,寫出第三人就是錯。

#### 生成提示只釘身份、不給情節

第一輪刻意不把官方劇情餵進提示,讓產出**獨立於帳本**,查證才不是球員兼裁判。
(查證做完、錯誤逐項記錄之後,第二輪才用官方事實去約束重生成。)

#### 通讀結果

英文 19 篇逐篇讀完:**通過 7、待外部查證 3、待修正 7、待重生成 2**,共 29 項問題。
中文批另外抓到:

- `Starzyński` 繁中開頭寫「**1941年**萬靈節前夕」,同一段後面又寫 1939-10-26 → 自相矛盾,官方是 1939
- `NARUTO` 繁中整篇是**漫畫劇情**(水木、伊魯卡、封印之書、搶鈴鐺、達茲納、波之國、再不斬與白、中忍考試),
  但官方只說「演少年篇、鳴人與夥伴一同成長」,且本作是**無台詞動作秀**
- `Charlie Brown Christmas` 兩語都寫成「**學校**的耶誕劇」(官方通稿是鄰里的);
  繁中還漏掉 **Linus 的演說**(全劇最著名的轉折),並寫出 **派伯敏特・佩蒂** —— 該角色 1966 年才登場,
  1965 年原作沒有她

#### 外部查證 6 齣

- `EDDA - A kör` —— 匈牙利維基**逐句吻合**,連結局「Fausztusz 當選→承認競選不實→暴動→Frédi 中槍→
  見到未出生的兒子→十年後」都對。但抓到 `Isteni Hang` 應為 **`az Univerzum`(宇宙)**;
  維基寫的是 2013 原版結局,本檔屬 2024 修訂版系列。
- `Két összeillő ember` —— 維基證實 1972 年一月在**廣播電台節目**相遇、熱戀十個月(一到十月)、
  Domján Edit 1972-12-26 自盡、Szécsi Pál 1974-04-30 過世,生成時序全對。
- `Blood Brothers` —— El País 加泰語劇評明寫「els papers del **Mickey** i l'**Eddie**」,
  證實加泰語版沿用英文原版角色名。
- `Na prochach` —— e-teatr 劇評逐一證實角色名,並確認生成**漏掉重要角色 Królowa opioidów**
  (由編舞者飾演,有整段無台詞的身體演出)。另記分類歧異:該劇評開頭寫「這不是音樂劇」,
  但官方標 SPEKTAKL MUZYCZNY、KPO 文件寫 Mikromusical → 採官方。
- `Piotruś Pan` —— 官方頁確認有 Jaś/Miś、無 Janek/Michaś;`Nibywalencja` 查無任何來源。
- `Carrie` —— 百老匯原卡司表確認角色名為本劇正典。

#### 我自己判錯的四次(全部記在帳本 `_overturned_suspicions`)

| 我以為錯 | 實際 |
|---|---|
| `Bitwa o tron` 的 Kordecki/Skarga/Katarzyna Wielka 是捏造 | 三個都在官方 Obsada 名單上,是**我抽表時截斷** |
| `Szécsi Pál` 自殺是未查證的推定 | 匈牙利維基:「1974-04-30 夜,最後一次已來不及阻止」→ 屬實 |
| `Łamignat` 的「强盗」與繁中的「丈夫」矛盾 | 波蘭維基:「Jaga 和她的丈夫、**強盜** Łamignat」→ 兩者都對 |
| `Tania Ruzs` 日期有誤 | atrapalo 官方 9/4–10/30,與我方一致;聚合站那筆是另一輪 |

這四次若照直覺改下去,就會把**正確的內容刪成錯的**,跟韓國批那 11 次同類。

#### 事實受限重生成:修好了原問題,也自己帶進三個新問題

第二輪把官方角色表餵進提示,原本的錯確實都修好了(Barker 不再寫成 Sackler、Dealer 與毒品線補回、
Jaś/Miś 對了、az Univerzum 對了、29 年對了)。但**副作用也很明確**:

- 模型開始**列名單**,把「Carole/Konferansjerka/Brenda」「Dennis Stone/dr van Hoff/dr Berkeley/Bob」
  這種連斜線的角色欄位原樣抄進正文,讀起來像節目冊(`Na prochach`、`Wypiór` 英文稿)
- `天堂邊緣` 英文稿**直接夾中文字**:「秀燕、何智 and 菲菲」
- `プリキュア` 因提示幾乎全是否定句,**英文 7 次全部回傳空白**(同一則提示在中文卻有產出)
- `プリキュア` 繁中把**日文漢字名改寫**:官方 **神白彩人**→神代彩人、**蜂針衣月**→蜂張樹、
  **百瀬昭彦**→百瀨明彥
- `Caperucita Roja` 繁中把主角**音譯**成「卡佩露西塔」,比第一輪的「小紅帽」差

已開第三、四輪修正,並把六條規則寫進 `docs/SYNOPSIS_SOP.md` §3.3。
匯出腳本另加三道機械關卡:三語筆數、空稿、**英文稿夾非拉丁字系文字**。

## [v2.113.3] - 2026-09-03 13:11

### 生成提示把「非拉丁字系」誤套到「拉丁字母的變音符號」,波蘭人名整批被拉平成 ASCII

補歐陸缺口時抓到的。`px_gen.py` 英文提示在 09-03 03:18(v2.112.0)為了擋韓日中文字
直接貼進英文稿,加了一句 `The English text must contain no non-Latin characters at all.`
模型把它讀成「不能有非 ASCII」,於是**波蘭文的 ł ń ó ś ż ź ą ć ę 全被拉平**:

| 官方寫法 | 生成寫成 |
|---|---|
| Stanisław | Stanislaw |
| Władysław | Wladyslaw |
| Starzyński | Starzynski |
| Wazówna | Wazowna |

實測三篇英文稿的變音符號數 **= 0**。人名寫錯就是資料錯。
已改成明確區分:要剝的是**書寫系統**(CJK/諺文/西里爾/希臘/泰/阿拉伯/希伯來),
**拉丁字母的變音符號一律照製作方的寫法保留**。

### 全庫回掃:既有資料沒有被污染

依「修症狀類 bug 必掃全庫同症狀」的規矩回掃 129 篇歐陸/德奧/法式/西葡英文簡介。

**先修正了自己的錯誤驗法**:第一版只數「有沒有變音符號」,得到 43% 是 0 —— 但那不算證據,
很多英文簡介本來就沒有需要變音符號的詞(Elisabeth 講 Sisi/Franz Joseph,本來就不用)。
改成驗「**該有卻寫成 ASCII**」後,7 筆命中裡 5 筆是 `Los Angeles`(英文本就不帶重音)、
2 筆是 `Nino`(Amélie 的 Nino Quincampoix、歌手 Nino Bravo,本來就沒有波浪號)——
**真實案例 0 筆**。

原因用 git 確認:那句是 09-03 03:18 才加的,而歐陸/德奧/法式/西葡各批都在 09-02 以前入庫,
所以只影響【09-03 03:18 之後產生的拉丁字系新稿】。韓國批不受影響(韓文本來就要羅馬拼音)。
...
- **The Gazillion Bubble Show**(New World Stages, NYC)—— 官網逐字讀過:
  Fan Yang 家族的泡泡藝術表演,賣點是雷射、音樂、把觀眾裝進巨型泡泡、18 項金氏世界紀錄。
  **全站沒有任何角色或劇情**,是 Off-Broadway 闔家綜藝秀。
- **Memory Card #2: Georgia Through Masterpieces of Music and Cinema**
  (Town Hall, NYC, 2026-11-29 單場)—— Town Hall 整季節目表全是演唱會/脫口秀/朗讀會;
  全網僅存的一筆資料是主辦的 Facebook 活動頁,標為「Theater event」。
  **查無任何來源稱它為音樂劇**,是喬治亞音樂電影精選節目。
...
全站缺劇情 110 組,**82 組是中國原創(0%,整批沒做)**,佔 75%。
非中國的 26 組原因已逐筆查證:歐陸批次按語言圈選、**波蘭語整塊沒做**(8 組)+ 匈牙利 1 組;
8 組是 09-02 之後才進檔期的新戲;4 齣韓國是官方零劇情資訊(已判定不寫);
其餘 5 組(Blood Brothers、On Your Feet!、Carrie、NARUTO、天堂邊緣)是該補的舊缺口。
已排除「知識庫有內容但 key 對不上」的可能:**0 組**,不是靜默漏接。

## [v2.113.1] - 2026-09-03 11:29

### 覆蓋率稽核報表的「資料時間」是寫死的,隔天重跑會印出昨天的日期

產一份新的三語覆蓋率稽核給使用者時發現:`scripts/qa/coverage_html.py` 的模板把
`資料時間 2026-09-02` 直接寫在 HTML 裡。今天重跑,副標仍印 09-02 ——
拿去當 status update 就是直接給出錯誤資訊,而且**沒有任何地方會報錯**。
改成 `--date`(預設取當天),另加 `--delta <fragment>` 讓一次性的「與上一份的差異」
用外部檔帶入,不再寫死進模板重蹈覆轍。

### 順帶查證出來的兩件事

**1. 覆蓋率的百分比不可直接跨日比較。** 分母是「當下在檔期的組數」,票務更新就會變。
用 `git worktree` 把 09-02 狀態拉出來跑同一支腳本:**繁中標題 377、簡中標題 377、
劇情 457、三語全滿 256 四項完全重現**,但分母(630 vs 636)與英文標題(455 vs 459)
對不上 —— 那份 HTML 產出時本機剛更新過場次,比 commit 多 6 組。
結論:**內容類數字可重現,檔期類數字本質上不可重現**,報表比較一律以組數為準。

**2. 09-02 → 現在少掉的 4 組簡介逐筆查證過,不是掉資料。**
`2026 midnight actor musician the choice`、`2026 wtof million loves` 是 v2.111.0
修掉的「城市後綴拆組」留下的重複組,內容已改掛正式組 `midnight knapman and wythe`、
`million loves`(兩組都在檔期、都有三語簡介);`a tenkes kapitanya`、
`curious case of benjamin button` 是真的下檔。

## [v2.113.0] - 2026-09-03 10:34

### 韓國 55 組三語簡介入庫 —— 但這批真正的產出是「我自己也會讀錯圖」這件事

61 組裡 55 組入庫、4 組判定不出簡介(見文末)、2 組先前已併入他組。
方法 B:Perplexity 生成為主體(語感),我逐齣對官方原文做事實校對,只修錯不改文風。

#### 抓到的錯,大宗不是情節寫錯,是「把原著/小說的人物寫成這個製作的角色」

- `휴남동 서점` 英簡把**小說才有的 지미·상수·민철**寫成台上角色 —— 我把 NOL 캐스팅 的「더보기」
  全部展開逐一讀,官方**只有 5 個角色**(영주/민준/승우/희주/정서)。
- `Club 설화` **三語共同**虛構了「까치와 까마귀」主持串場。官方 CHARACTER & CAST 只有
  호랑이/도깨비/놀부/구미호 四隻。三語一致不是佐證,是同源痕跡。
- `늦봄의 길` 英文生出「**Peimun**」—— 這根本不是可辨識的韓文名字。官方「주요 등장인물」正好 5 人。
- `청사초롱` 英文憑空生出五名員工(Imo/Icheol/Gonji/Yangpyeong/Gapyeong),而官方真正的
  **수두매** 反而不見了;繁簡還都把結局寫成「婚禮辦成、全劇以婚宴收束」,官方其實停在
  「윤덕 接下委託、청사초롱 開始忙碌」。
- `파랑새` 三語都寫成原著版的「仙女生病的女兒」—— 本製作官方明寫**생병的是妹妹 미틸**;
  七個靈魂也都漏了**빛(光)**,英文還多出一個官方沒有的 Milk。
- `미드나잇` 簡中寫「蘇聯統治下的**巴庫**」;나무위키 明載巴庫是**原作劇本**《Citizens of Hell》
  的設定,本劇只說「소련 어딘가」。
- `사랑해져라` 繁中寫「父**子**」、簡中寫「小**女孩**」—— 兩邊互相矛盾。官方海報顯示 가람 是女孩。
- `켈로` 三語都漏掉主角:官方開場就是「**열세 살 강화도 소녀 용주**」。
- `런던레코드` 繁簡都把約翰寫成「主唱」,官方是樂團 **Hot play 的團長兼吉他手**。

#### 🚨 我自己讀官方長圖轉錄進帳本的角色欄,有系統性錯誤

這批最該記下來的一項。已查出 **5 齣**並全部回原圖逐字重錄:

| 齣 | 我記成 | 官方實際 |
|---|---|---|
| `좀비` | 아**빠**(爸爸) | 아**들**(兒子)—— 簡中因此寫出「另一位『爸爸』」 |
| `헤어드레서` | 兩位主角台詞**對調**;한**금**단;「연우(오마례?)」 | 오마례↔최예화 台詞相反;한**곱**단;이연우 |
| `오싹한 알바` | 四句台詞**錯三句** | 並漏掉「알bar 在 **20cm 유리벽** 內」這個關鍵設定 |
| `놐놐놐` | 「**가옥**학교」「소스코롤」 | 「**기숙**학교」「스스로를」 |
| `Club 설화` | 놀부·구미호 台詞誤植 | 已照 26008571_05/06 重錄 |

**這會污染重生成 prompt** —— 오싹한 알바 就是先重錄角色欄、重建 prompt 之後才生對的。

#### 重生成 25 篇(三輪)

R1 主批判定「寫成別的戲」的 13 齣;R2 逐句比對後同樣不合格的語言;
R3 人工通讀時新查出的 청사초롱/파랑새/오싹한 알바/his story 簡中。
`헤어드레서` 三語補回完整弧線(義大利留學→百貨公司美容院→**軍資金傳遞據點**→예화告發→
**연우被槍決**→**예화縱火同歸於盡**→獄中日本人相助→朝鮮最大髮型秀)。

#### 外部查證:24 齣逐一實搜,分級重算

先前回報的「5 源、7 源」是**灌水**——那多半是同一張官方詳情圖的海報/SYNOPSIS/CAST/INFORMATION
四個段落,依 SOP §3.1 規則一同源不算佐證。只數**獨立於主辦方**的來源後:

- **A 多源交叉(≥2 獨立外部)48 齣 / B 單一外部 4 / C 僅官方 3**
- 剩下 7 齣全部**已實搜確認**,不是沒查:헤어드레서·생텍쥐페리·충분한 이별真的查無;
  벚꽃동산搜到的全是別的製作的契訶夫話劇;사랑을 노래하다只有樂團自家 IG。
- 外部來源還要再分層:`오셀로와 이아고` 有 6 個外部來源,但**全部只寫公演消息、沒有一篇談劇情**,
  已在帳本標明劇情仍屬官方單一來源。

搜尋補回不少三語漏掉的官方事實:**켈로是真人真事**(KLO 部隊與少女情報員真實存在)、
**크리스마스 캐럴三精靈由一名演員分飾**、**김종욱 찾기是三人劇且為韓國「멀티맨」風潮開創者**、
**푸른 꽃的올리브與마틸데同一演員分飾**、**오싹한 알바是百老匯式全唱**、
**바람으로의 여행唱的全是金光石**、**만천명월是惠慶宮花甲之行**、
**1457的鬼怪是「도깨비 삼남매」三兄妹**。

#### 懷疑 ≠ 有錯:11 次被推翻,1 次先刪才發現刪錯

被推翻的包括 `Robert Alexander Schumann` 的中間名(德文維基)、`빨래` 的「빵」是真角色
(제일서점 사장,본명 엄훈성)、`london record` 的「Charlie」、`종의 기원` 的「Yumin」、
`widerstand` 的 Hagen/Jasper、`카페인` 的「끝에서 두 번째 여자」。
唯一誤刪:`늦봄의 길` 的「지영」—— 부천문화재단官方 SYNOPSIS 寫著
「지영이의 손에 이끌려 온 루다」,已復原。

`달콤한 위로 초코파이` 則是官方與報導不一致:2025 年兩家報紙寫「동오제빵소」,
官方 2026 詳情頁逐字是「명진제빵소」→ **採官方、把差異記進 `ledger.discrepancy`**。

#### 順手修掉的工具缺陷

- `scripts/px_gen.py`:`ask_once()` 沒有 `try/finally`,goto/type 逾時時分頁會留在瀏覽器裡。
  累積幾個之後**下一次 `connect_over_cdp` 就卡死** —— 症狀是 ws 明明「`<ws connected>`」
  卻仍 Timeout 180000ms。我先誤判成並行競爭,序列化後還是掛;清掉 4 個遺留分頁才恢復。
- 繁簡譯名一致性全掃(繁→簡轉換後比對),修掉 `카페인` 的 지민/정민 與
  `말하지 못한 사랑` 的 서연 兩組不一致;另修 13 處我手改時帶進去的半形標點。

#### 判定「不出簡介」的 4 齣(待確認)

官方完全沒公開劇情,三語寫出來的全是模型自行補的:

- `나의 첫사랑 레시피` —— 官方只有一句標語;**繁中寫成女性、簡中寫成男性**,同一角色互相矛盾
- `물속의 달 - 오필리어` —— 官方只有一張海報;三語寫出整套《哈姆雷特》,但**沒有任何來源說本劇
  改編自哈姆雷特**,而且分級是滿 7 歲
- `고백` —— 官方只有藝術節節目表 + 海報(唯一線索是「被鮮花包圍的病床」)
- `조선셰프 한상궁` —— 官方只到「全州拌飯誕生的故事」;三語生出對手「崔尚宮」與料理競賽

#### 文件

`docs/SYNOPSIS_SOP.md` 補入 §3.2「韓國批次補上的八條規則」。

---

## [v2.112.1] - 2026-09-03 04:24

### P4 逐篇查證完成:59 齣 × 三語 = 177 篇,我本人一篇一篇讀完並對照官方原文

這一版【沒有】把任何簡介放上網站 —— 它記錄的是查證結果本身,存在
`scratchpad/kr/verify_log.json`,每一筆都寫明:我讀了哪三篇、對照的官方逐字原文是什麼、
判定為何、以及要怎麼修。

| 判定 | 齣數 |
|---|---|
| publish(三語都正確) | 19 |
| fix(有明確事實錯誤,已逐條記下最小修正) | 23 |
| regenerate_all(三語都寫成別的故事) | 6 |
| regenerate_zh / cn / en(單一語言寫錯) | 4 / 2 / 1 |
| reject(官方零劇情,誠實地寫不出來) | 4 |

#### 🚨 六齣被整批寫成【另一部戲】

模型在找不到資料時不會說「不知道」,它會拿一部**題材相近的知名作品**頂上:

| 齣 | 產出寫的是 | 官方其實是 |
|---|---|---|
| Million Loves〈백만사〉繁中 | **《媽媽咪呀!》**(唐娜、安潔拉、三個舊情人、婚禮) | 橫跨韓國近現代的四十餘首金曲串成的平凡人群像 |
| R＆J 簡中 | **Joe Calarco《Shakespeare's R&J》**(四名寄宿學校男生偷排禁書) | 二十位演員的羅密歐與茱麗葉,還新增了三女巫 |
| 생텍쥐페리 繁中 | **《小王子》整本**(B-612、玫瑰、狐狸與馴養) | 聖修伯里【本人的生平】,小王子是他的另一個自我 |
| 이머시브 좀비 簡中 | 過氣演員劉華、丈夫澤洙、絕症兒子鐵根 | 官方人物表裡一個都沒有(是 헌터 與 수지) |
| 달콤한 위로 초코파이 三語 | 東五麵包店、25 年前的火災、父親失智 | 명진제빵소的父子 + 88 豆芽湯飯店母女 |
| 목마와 숙녀 三語 | 咖啡館裡老歌勾起回憶的泛泛框架 | 현구 與 선희:專輯成功後疏遠、她病逝、他靠音樂重生 |

📌 這些都是【同源痕跡】:三語一致不代表為真,因為三個版本出自同一個模型。

#### 抓到的具體事實錯誤(節錄)

- **《그날들》三語全錯**:都寫「二十年後 / 2012 / 建交 20 週年」,官方 2026 版逐字是
  **「2022년… 한중 수교 30주년」**(舊版本才是 20 週年)
- **《푸른 꽃》簡中**寫 2026 年,官方是 **2025년**
- **《파랑새》三語**都照原著寫成「仙女的女兒生病」,官方是 **「몸이 아픈 미틸」——生病的是妹妹本人**
- **《M. Butterfly》英文**把妻子寫成 **Helga**(黃哲倫原作話劇的名字),本製作是 **아녜스/Agnes**
- **《벚꽃동산》繁中**把**櫻桃園寫成「櫻花園」**(韓文 벚꽃 字面是櫻花,但契訶夫原作華語定名是《櫻桃園》)
- **《Club 설화》**的 **설화=說話/傳說**,繁中譯「雪話」、簡中譯「雪花」
- **《바람으로의 여행》英文**把安企部高層的父親安到了 은영 身上,官方是 **고은**
- **《켈로》三語**都把主角弄丟了 —— 官方是【江華島十三歲少女 용주】,因親眼看見轟炸中村民死去而從軍

#### 英文的系統性缺陷:普通名詞被當成人名羅馬拼音

我在前一版加的「非拉丁字系一律羅馬拼音」被過度套用。官方人物表常直接用普通名詞當角色名:

`Geunyeo`(그녀=「她」)、`Geu Namja/Geu Yeoja`(那個男人/女人)、`Horangi`(老虎)、
`Gumiho`(九尾狐)、`Kkachi`(喜鵲)、`Najdokkaebi`(白日鬼怪)、
`Cheotsarang Chatgi Jusik Hoesa`(尋找初戀股份有限公司)、
`Yosep / Sebang Yohan / Garot Yuda`(約瑟/施洗約翰/加略人猶大)、
`Su / Jonsi / Beoman`(歐·亨利原作的 Sue / Johnsy / Behrman 繞一圈被拼壞)、
`Delphoi / Athina`(應是 Delphi / Athens)。

已修 `px_gen.py`:只有專有名詞拼音,普通名詞一律意譯;英語已有標準名稱的地名人名用英語寫法。

#### 🚨 兩次我差點刪掉正確內容,查證後推翻自己

- **《박열》**:我要刪掉審判後的史實(改判無期、文子獄中身亡、1945 出獄)。查了四個來源後發現
  **那正是這齣戲的結局** —— DCinside 有雷心得逐字「이 극의 결말은 1945년에 박열이 석방되어
  후미코의 무덤을 찾는 것」。
- **《광화문연가》**:我懷疑學運情節是編的。五個來源證實**確實在戲裡** —— 뉴시스逐字
  「독립정권의 군홧발에 짓밟히는 수아를 지켜주지 못했다는 자책감에 시달리다 군에 입대한다」。

📌 教訓已記進帳本:**「官方文案沒寫」≠「劇裡沒有」**。官方 SYNOPSIS 常刻意止於高潮前,
要判斷「超出官方文本」是否為錯,必須另外查【看過戲的人】的紀錄。

#### 另外修的兩個 px_gen 清理漏洞

- 韓國售票站 **`ticket.yes24`** 因為結尾是數字而躲過網域過濾,在 5 篇裡被當成正文留下
- 來源名會**黏在句末**而不是自成一行,逐行過濾抓不到 → 成文後再做一次行內清除
- 兩者都已修並雙向回歸測試(不會誤砍 `No.5 Main St.` 或 `2026.10.10`)

⚠ 下一步:13 齣需要重生成(6 齣三語全重、4 齣繁中、2 齣簡中、1 齣英文),
23 齣套用最小修正,4 齣不出簡介。全部完成後才會入庫上站。
