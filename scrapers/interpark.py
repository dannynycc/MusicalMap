...
def strip_season_year(t):
    """剝掉開頭的年份「檔期標記」——但只在確定它不是劇名本身的時候。

    Interpark 很常把年份當成當季版本的前綴:「2026 Musical 〈MIDNIGHT : ANTLERS〉」、
    「2026뮤지컬 〈미드나잇:액터뮤지션〉」、「2026 〈Midnight : Actor Musician〉」。
    留著年份會讓【同一齣】被拆成好幾個 group(2026-09-02 實測 MIDNIGHT 一齣裂成 4 組、
    1457 소년 잠들다 裂成 2 組)。

    🚨 但年份【也可能就是劇名】(1776、1789、1984、BACK TO 1931…),無腦剝會毀掉劇名。
    所以只在名稱裡另外有【書名號 〈〉/《》 或 ＇＇ 包住真正劇名】時才剝 —— 那時年份
    位在書名號之外,身分明確是檔期前綴。1776 這類劇名沒有書名號,規則碰不到它。
    """
    if re.search(r"[〈《＇']", t) and re.match(r"^\s*(19|20)\d{2}\s*년?", t):
        t = re.sub(r"^\s*(19|20)\d{2}\s*년?\s*", "", t)
    return t
...
def clean_title(name):
    ...
    t = strip_tour_city(t)
    # 開頭方括號裡是【藝術節/單元名】而不是劇名(「［2026 리:바운드 축제］1457, 소년 잠들다」)。
    # 同一齣戲被某個藝術節邀演時會多這一層,留著就會跟本體拆成兩組。只認明確的
    # 축제/페스티벌/FESTIVAL 字樣,不做通用剝除(方括號裡也可能是劇名的一部分)。
    t = re.sub(r"^\s*[\[［][^\]］]*(?:축제|페스티벌|festival)[^\]］]*[\]］]\s*", "", t, flags=re.I)
    t = strip_season_year(t)
    # 售票席次型態後綴(비지정석=自由席/지정석=對號座):是票種不是劇名,留著會拆組。
    t = re.sub(r"\s*(비지정석|지정석)\s*$", "", t)
