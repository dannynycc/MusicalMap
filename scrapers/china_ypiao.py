"""China multi-city musicals — discovery via the 有票网 (ypiao.com) aggregator.
...
ypiao is a THIRD-PARTY aggregator, so we treat it as a discovery source only:
 • keep only items whose title is a staged musical  音乐剧《…》  (drops the concerts
   / 演唱会 / 钢琴音乐会 that share the listing pages),
 • take title / city / venue / run-dates from the listing row,
 • resolve coordinates ONLY from data/cn_venues.json (Google-geocoded, GCJ→WGS84
   already applied). A show whose venue we can't map to a known coord is SKIPPED —
   better to omit it than to place it at a guessed/wrong location.
 • posters: ypiao detail pages carry only placeholders, so none here; registered
   works (e.g. 剧院魅影 → Phantom) inherit a poster by group in build_shows.py.
...
CN_CITY_EN = {"北京": "Beijing", "上海": "Shanghai", "广州": "Guangzhou", "深圳": "Shenzhen",
              "武汉": "Wuhan", "杭州": "Hangzhou", "南京": "Nanjing", "成都": "Chengdu",
              "苏州": "Suzhou", "西安": "Xi'an", "重庆": "Chongqing", "天津": "Tianjin"}
...
def load_venue_coords():
    ...
    stripped, trailing hall qualifier removed, so '天桥艺术中心-大剧场' matches
    '北京天橋藝術中心 大劇場'."""
...
def _fold(s):
    s = re.sub(r"[\s\-－—·]", "", t2s(s))
    return re.sub(r"(大剧场|歌剧厅|音乐厅|中剧场|小剧场|歌剧院|大剧院大剧场)$", "", s)
...
def main():
    ...
    for url in pages:
        ...
        for href, inner in re.findall(r'<a[^>]+href="(/t_\d+)/?"[^>]*>(.*?)</a>', html, re.S):
            ...
            if "音乐剧《" not in txt and "音樂劇《" not in txt:
                continue
            title = re.search(r"《([^》]+)》", txt)
            venue = re.search(r"场馆[:：]\s*(\S+)", txt)
    ...
    for href, (title, venue, city_cn, dstr) in rows.items():
        ...
        shows[sid] = {
            ...
            "venue": venue, "city": CN_CITY_EN.get(city_cn, city_cn or "China"),
