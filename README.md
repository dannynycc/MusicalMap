...
> **近期(v1.26–v1.33)**:海報卡 ♥**標最愛**(`add_fav.sql`)；**刪除帳號**(帳號設定→危險操作,GDPR,`add_delete_account.sql`;⚠️ **實機失敗待修**——疑 SQL 直刪 auth.users 權限問題,正解改用 Edge Function+service_role,見 CHANGELOG v1.33.2)；搜尋併入 catalog `search` 欄,簡體劇名/劇院名(女巫前传/爱知县艺术剧场)也搜得到；貨幣/persona/城邦地名等 i18n 與邊界修正；星級評分/詳情海報**鍵盤可及**；**手機地圖圖卡改「底部彈出 sheet」**(貼底、可上下捲、觸碰不消失——修 autoPan 觸發 markercluster re-cluster 導致「閃一下消失」;⚠️ 仍有超框改進空間待續)。詳見 CHANGELOG。
