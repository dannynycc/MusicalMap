...
- ✅ **多平台分潤框架（`MM_CONFIG.AFFILIATE` + `affiliateUrl()`，見 `docs/DESIGN_affiliate.md`）**：render 時把外連售票 URL 依 host 包成分潤連結（資料層只存乾淨 URL，換 ID = 改 config 一行）。支援 Impact／Partnerize／Awin／tmpl(Sovrn 等)多網絡;每個程式 dormant，填碼即生效。**Ticketmaster（Impact，~600 齣）已 live**。各平台 2026-06-23 逐一查證(見 `docs/DESIGN_affiliate.md`):**ATG（Partnerize，219，可申請）／Broadway Direct（Awin 28987）** 待填碼;**TodayTix 直接計畫已關**，僅能經 **Sovrn Commerce**(Open)變現。**TodayTix 改導 matcher 已上線**（`scrapers/todaytix.py`，逆向其開放 API，掃 13 城 musicals → 對應到我們的劇 → 掛 TodayTix deep link，101 筆，排在票務最前）。
