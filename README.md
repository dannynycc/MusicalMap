...
> **近期(v2.49–v2.50)**:me/u 統計頁「你是什麼樣的劇迷?」大改版——**六邊形戰力圖**(`js/mm-persona-radar.js`)六軸=**劇院常客／嘗鮮作品／劇種涉獵／跨國足跡／狂粉劇迷／音樂劇齡**,各軸 0–100 分且**全用對數前載** `fl(x,cap)=100·ln(1+x)/ln(1+cap)`(頭幾部就有感、越後越難;絕不用線性),0 場給鼓勵句、<5 場加成長提示、頂點半徑設 floor 防塌陷但標籤/清單仍顯示真實值;配色六軸各異(紅/青綠/草綠/深藍/紫/洋紅,避開品牌金)、格線疊層看得出分數級距。**成就徽章 17 族**(`data.js` badges(),各族多級續階門檻讓高活躍者永遠有下一級:場次/國家/城市/作品/N 刷/雙刷日/連年/五星/劇評/心頭好/劇種/常客劇院/馬拉松月/資深/跨城/口袋名單),圖示為手繪 SVG(`js/mm-badge-icons.js`)。未來場次卡改**薄紗白**(不再壓黑)。規則使用者訂死、隨時可能調整。
