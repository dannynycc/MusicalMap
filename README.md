...
- ✅ **多平台分潤框架（`MM_CONFIG.AFFILIATE` + `affiliateUrl()`，見 `docs/DESIGN_affiliate.md`）**：render 時把外連售票 URL 依 host 包成分潤連結（資料層只存乾淨 URL，換 ID = 改 config 一行）。支援 Impact／Partnerize／Awin／tmpl(Sovrn 等)多網絡;每個程式 dormant，填碼即生效。**已 live 變現**:**Ticketmaster**（Impact，~600 齣）+ **TodayTix**（經 **Sovrn Commerce / VigLink**,~1-2%,**101 筆**改導連結,排票務最前)。待填碼:**ATG**（Partnerize,219,可申請)、**Broadway Direct**（Awin 28987)。**TodayTix 直接計畫已關**,故走 Sovrn(`redirect.viglink.com?key=…&u={url}`)。各平台 2026-06-23 逐一查證,詳見 `docs/DESIGN_affiliate.md`;改導用 `scrapers/todaytix.py`(逆向 TodayTix 開放 API,掃 13 城 musicals 對應)。
