...
> 目標:把地圖連出去的售票連結變成可抽成。技術面已就緒(`js/config.js` 的 `MM_CONFIG.AFFILIATE`
> 設定 + `js/app.js` 的 `affiliateUrl()`),**只差申請帳號拿 ID**。拿到 ID 後填進 `AFFILIATE` 即生效。
> **現況(2026-07 更新)**:Ticketmaster 走 Impact live;todaytix/londontheatre/atgtickets/broadway-show-tickets/atrapalo 皆已經 **Sovrn catch-all** 過渡變現(`net:"tmpl"`)——本檔的 Partnerize/Awin 申請屬「升級為直接計畫」,非從零開始。
>
...
**現況**:ATG 連結已先經 Sovrn 過渡變現(`js/config.js:49`,註記 interim);Partnerize camref 到手後升級為直接計畫(佣金較高)。
...
→ ~~要抽成必須加入 TodayTix Group 聯盟,走 Impact~~ **(已過時:後查證 TodayTix 直接計畫已關閉,見 `DESIGN_affiliate.md` §5 收尾教訓)。現況:londontheatre.co.uk 與 todaytix.com 都走 **Sovrn(`tmpl`)** 變現(`js/config.js:46-47`),本節 Impact 步驟不再適用,保留作歷史。**
...
把以下貼給我(有哪個給哪個):
- ~~Impact:追蹤網域、Account ID、(Ticketmaster 的)Ad ID + Campaign ID~~(✅ 已填,live)
- Partnerize(ATG):camref(唯一仍待索取項;到手前 ATG 走 Sovrn 過渡)
- ~~Awin(LondonTheatre)~~(已不需要:londontheatre 走 Sovrn,見 §3)

我會填進 `js/config.js` 的 `AFFILIATE`、用幾個真連結測試會不會正確包裝,再 commit 上線。
