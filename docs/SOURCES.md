...
| **西班牙全境(主力)** | atrapalo.com/entradas/musicales | `atrapalo.py` | 全西班牙 39 城~147 齣;Playwright 過 Fastly 挑戰→curl_cffi 抓頁面 JSON-LD(劇名/場館/**經緯度**/檔期/價/海報),**任一頁被擋就自動退回瀏覽器重抓**;**Sovrn 分潤**(merchant 53900);重疊劇主源,EXTRA_PRODUCTS 補非-musicales 分類 | 2026-08-20 |
| 東歐(匈牙利等) | jegy.hu 等 | `easteurope.py` | 匈牙利/捷克等地方場館,EE 區 | 2026-06-13 |
| 義/瑞/荷/波/挪/奧/中東 | teatro.it / showtic(瑞典) / stage(荷蘭) / ebilet(波蘭) / 挪威 / VBW(奧地利) / platinumlist(中東) | `italy.py` `sweden.py` `netherlands.py` `poland.py` `norway.py` `austria.py` `middleeast.py` | 各國原創/巡演;德奧(VBW)→德奧 tag,其餘多→歐陸其他。**ebilet.pl 上了 DataDome(2026-08-24)**:429 的 body 是 JS 挑戰頁(`dd_referrer`),封鎖為 IP 級,urllib/curl_cffi/headless Playwright/真 Chrome 當天全被擋;`poland.py` 已加「挑戰頁→中止整輪」與「事件頁抓取失敗率 >25%→不覆蓋舊檔」兩道守門(在此之前無守門,曾以 4 筆殘缺覆蓋 12 筆好資料、悄悄弄丟 8 檔)。若常態化,下一步走 atrapalo.py 那套 Playwright 取 clearance cookie。**teatro.it 結構限制(2026-07-14)**:JSON-LD subEvent 每站每年只披露一場「代表場」,多天檔期會被砍成單日(Carcano 官方 12/23~27 只給 12/23)——重點站以官方頁 override 修正(先例 it-349cb901),其餘單日檔期視為「至少演出日」低估 | 2026-06-13 |
