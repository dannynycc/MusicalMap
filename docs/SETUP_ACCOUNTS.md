...
3. 後續加欄位的 migration(既有資料庫補跑即可,皆 `add column if not exists`,可重複執行):
   ...
   - `supabase/add_delete_account.sql` —— **v1.30.0 刪除帳號(GDPR 被遺忘權)**：`delete_my_account()` SECURITY DEFINER RPC,只刪呼叫者本人(auth.uid())的 sightings/profile/handle_aliases(cascade)+ venues.created_by 設 null(保留社群劇院)+ auth.users。已套用,但 ⚠️ **實機刪帳號仍失敗**(疑最後 `delete from auth.users` 權限問題,Supabase 該表由 supabase_auth_admin 擁有)——**待修**:改用 Edge Function+service_role 呼叫 `auth.admin.deleteUser()`。見 CHANGELOG v1.33.2。
