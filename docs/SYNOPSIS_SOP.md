...
### 3.4 中國原創批次(2026-09-04, 76 組 = 69 主批 + 7 補充)補上的規則
...
**十三、帳本抽取可能抓到【同一頁的別的段落】,而且看起來完全正常。**
`寻找李二狗` 我以為抓的是百度百科「剧情简介」段,實際抓到的是同頁「艺术特色」段。
兩段都是通順的中文、都在講這齣戲,肉眼看不出異常;結果**整組的前提**
(主角常衛國、爺爺的回憶錄、1950 鴨綠江畔)全部沒進 prompt。
發現它的不是任何守門,是**內部矛盾**:劇名叫《尋找李二狗》、官方卡司表也有「李二狗」,
稿子卻整篇沒有他。
→ 抽取後要**回頭確認抓到的段名**;→ 通讀時把「劇名/官方角色表裡有、稿子裡沒有」當成紅旗;
→ 發現一組出錯,**同來源的其他組全部重抓比對**(本次另外四組:兩組逐字相符、兩組是段名不同,
只有一組真的錯)。

**十四、簡中稿要掃繁體字殘留——但只有「資料來源是繁體」的組會中招,抽樣抓不到。**
`ne zha` 簡中寫出「鬧海抗爭」「蓮花重生」。不是模型亂寫:**帳本裡那一組的資料是用繁體存的**,
而 prompt 又要求「專有名詞一字不改照抄」,模型照做了。其他 75 組一個都沒有。
→ `scripts/zhs_trad_scan.py` 全掃。🚨 OpenCC `t2s` 有誤報:`吒→咤`(哪吒繁簡同形),
跟著轉反而錯,誤報要寫進 `SKIP` 並註明理由。

**十五、同一篇裡的用詞重複,守門與逐句對照都查不到,要另外掃。**
`牡丹亭` 同一句兩次「傳奇」、`日出渤海` 相鄰兩句兩次「渤海軍區教導旅」、
`荒原` 相鄰兩段兩次「復仇並非終點」、`ne zha` 三個情節座標列兩次。
→ `scratchpad/cn82/dup_scan.py`。🚨 **只報告不判定**:人名與核心物件本來就該重複,
收尾段呼應主題詞更是 prompt 要求的結構。順序仍是**先讀出這一類、才寫成掃描**。

**十六、「修 A 引入 B」在這批發生六次,其中一次是修正的修正。**
簡中第二輪重讀找到 8 個問題,**7 個是我自己插入的句子造成的**:刪掉憑空補的地點後
整句沒了謂語、補進去的名詞片語無法與既有的疑問子句並列、補進去的詞與收尾段撞字。
→ 每一條定點修正套用後,**連左右鄰居一起重讀**,不能只看那一句自己通不通。

**十七、自己的工具會因為「在鍵值集合裡混入註記鍵」而全面炸掉。**
我在 `verify_log.json` 加了 `_overturned_suspicions`(值是字串字典),
`verify_log.summary()` 與 `st.py` 兩支讀取端都假設「每個頂層鍵都是某組的判定」,當場拋
`TypeError`。→ 註記鍵一律以 `_` 開頭,**每個讀取端都要有同一條排除規則**。
...
- 🚨 **`kb_merge.py` 每次執行都會【清空】`kb_merge.last.log`**（見該檔 L62，所以叫 *last*.log）。
  一輪要跑很多次時（本批三語 × 主批/補充/重生成 = 9 次），逐行讀 log 這道檢查會**只讀到最後一次**。
  → 每跑完一次立刻 `cat ... >> 合併檔`，最後對合併檔驗：對應行數 = 組數 × 語言數、
  每個目標鍵都是真 group、每個鍵剛好出現「語言數」次、沒有長得像 prompt 的垃圾鍵。
- 🚨 **子集重生成要用獨立輸出後綴**（`mk_regen.py <lang> <pick.json> <suffix>`），
  並確保套用工具認得該後綴（`apply_fix.py` 的 `suf` 欄位）——否則修正會**靜默套到主批檔上**。
  子集稿完成後，要把該組從原批次的 results/order **同時同索引移除**（兩者是位置對應，
  只刪一邊會讓其後每一筆都掛到別組），移除後逐筆驗證位置對應仍正確。
